...
DOWNLOAD_ROOT="${DATA_DIR}/downloads/nasa_power_daily_climate"
FILTERED_ROOT="${DATA_DIR}/filtered/nasa_power_daily_climate"
INDEX_ROOT="${DATA_DIR}/index/nasa_power_daily_climate"
SAMPLES_ROOT="${DATA_DIR}/samples/nasa_power_daily_climate"
LOG_ROOT="${DATA_DIR}/logs/nasa_power_daily_climate"
        ...
        for parameter_id in parameter_ids:
            ...
            for field, value in [
                ...
                if int(stats_row[field]) != value:
                    raise SystemExit(
                        f"stats mismatch for {location_id} {parameter_id} {year} field {field}: "
                        f"stats={stats_row[field]} raw={value}"
                    )
            if stats_row["start_date"] != start_date:
                raise SystemExit(
                    f"start date mismatch for {location_id} {parameter_id} {year}: "
                    f"stats={stats_row['start_date']!r} raw={start_date!r}"
                )
            if stats_row["end_date"] != end_date:
                raise SystemExit(
                    f"end date mismatch for {location_id} {parameter_id} {year}: "
                    f"stats={stats_row['end_date']!r} raw={end_date!r}"
                )
...
for (location_id, parameter_id), value_count in sorted(group_counts.items()):
    ...
    for series in series_defs:
        ...
        expected_records[(series["series_id"], sample_slug)] = {
            "dataset_id": "nasa_power_daily_climate",
