...
items=obj
meta={"coinpaprika_rank": ("uint", 16, "H"), "coinpaprika_price_usd": ("float", 64, "d"), "coinpaprika_market_cap_usd": ("float", 64, "d"), "coinpaprika_volume_24h_usd": ("float", 64, "d")}
...
(filter_dir/'ingest_stats.json').write_text(json.dumps({"dataset_id":"coinpaprika_tickers","rows_total":len(items),"rows_skipped":skipped}, indent=2, sort_keys=True) + '\n', encoding='utf-8')
