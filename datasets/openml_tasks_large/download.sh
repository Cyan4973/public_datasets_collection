...
BASE_URL="${OPENML_TASKS_LARGE_BASE_URL:-https://www.openml.org/api/v1/json/task/list}"
PAGE_SIZE="${OPENML_TASKS_LARGE_PAGE_SIZE:-1000}"
TARGET_RECORDS="${OPENML_TASKS_LARGE_TARGET_RECORDS:-20000}"
MIN_RECORDS="${OPENML_TASKS_LARGE_MIN_RECORDS:-10000}"
REQUEST_DELAY="${OPENML_TASKS_LARGE_REQUEST_DELAY_SECONDS:-1}"
UA="openzl-public-datasets/1.0 (numeric dataset collection)"
OUT="$DOWNLOAD_DIR/tasks.json"
...
case "$PAGE_SIZE" in ''|*[!0-9]*) echo "OPENML_TASKS_LARGE_PAGE_SIZE must be an integer" >&2; exit 1 ;; esac
case "$TARGET_RECORDS" in ''|*[!0-9]*) echo "OPENML_TASKS_LARGE_TARGET_RECORDS must be an integer" >&2; exit 1 ;; esac
case "$MIN_RECORDS" in ''|*[!0-9]*) echo "OPENML_TASKS_LARGE_MIN_RECORDS must be an integer" >&2; exit 1 ;; esac
if [ "$PAGE_SIZE" -le 0 ] || [ "$TARGET_RECORDS" -le 0 ] || [ "$MIN_RECORDS" -le 0 ]; then
  echo "OPENML_TASKS_LARGE_PAGE_SIZE, OPENML_TASKS_LARGE_TARGET_RECORDS, and OPENML_TASKS_LARGE_MIN_RECORDS must be positive" >&2
  exit 1
fi
if [ "$MIN_RECORDS" -gt "$TARGET_RECORDS" ]; then
  echo "OPENML_TASKS_LARGE_MIN_RECORDS cannot exceed OPENML_TASKS_LARGE_TARGET_RECORDS" >&2
  exit 1
fi

TMP_ROOT="$DOWNLOAD_DIR/tmp.$$"
TMP_PAGES="$TMP_ROOT/pages"
TMP_OUT="$TMP_ROOT/tasks.json"
...
obj = json.load(open(sys.argv[1], encoding="utf-8"))
rows = (obj.get("tasks") or {}).get("task")
if not isinstance(rows, list):
    raise SystemExit("bad OpenML task page: missing tasks.task list")
print(len(rows))
PY
)"
...
for page in sorted(pages_dir.glob("page_*.json")):
    ...
    page_rows = (obj.get("tasks") or {}).get("task")
...
if len(rows) < min_records:
    raise SystemExit(f"only {len(rows)} unique tasks < OPENML_TASKS_LARGE_MIN_RECORDS={min_records}")

payload = {
    "format": "openml_tasks_large_combined_v1",
    "source": base_url,
