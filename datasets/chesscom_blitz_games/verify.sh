...
ALLOWED = {"chesscom_blitz_rating_u16", "chesscom_blitz_plies_u16"}
...
for fam, samples in by_family.items():
    ...
    if {s["bit_width"] for s in samples} != {16}:
        raise SystemExit(f"family {fam} is not u16")
...
for r in primary:
    ...
    if len(set(unpack(p, int(r["value_count"])))) <= 1:
