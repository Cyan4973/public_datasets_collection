...
for row in obj:
    try:
        ...
        vals['gitlab_tag_count'].append(len(row.get('tag_list',[])))
    except Exception:
        skipped += 1
rows=[]
for sid,(kind,bits,code) in meta.items():
    ...
    rows.append({'dataset_id':'gitlab_projects','series_id':sid,'sample_path':out.relative_to(data_root).as_posix(),'numeric_kind':kind,'bit_width':bits,'endianness':'little','element_size_bytes':bits//8,'sample_size_bytes':out.stat().st_size,'value_count':len(values)})
(filter_dir/'ingest_stats.json').write_text(json.dumps({'dataset_id':'gitlab_projects','rows_total':len(obj),'rows_skipped':skipped},indent=2,sort_keys=True)+ '\n',encoding='utf-8')
