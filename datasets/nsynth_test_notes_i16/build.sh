...
for wav_path in wavs:
    ...
    if sample_min == sample_max:
        raise SystemExit(f"degenerate WAV payload: {wav_path}")
    ...
    row = {
        ...
        "sample_shape": [frames],
        "sample_axes": ["time"],
        "natural_record_kind": "nsynth_note_wav_file",
        "note_id": note_id,
        "framerate": framerate,
    ...
    rows.append(row)
    records.append({"note_id": note_id, "frames": frames, "bytes": len(payload), "framerate": framerate, "instrument_family": row["instrument_family"], "pitch": row["pitch"], "min": int(sample_min), "max": int(sample_max)})
