...
for r in primary:
    ...
    if any(b <= 32 or b >= 127 for b in data):
        raise SystemExit(f"unexpected non-printable symbol byte in {r['sample_path']}")
    if checked < 500:
        if len(set(data)) > 1:
            nonconstant += 1
        checked += 1
...
print(
    ...
    f"total_values={sum(counts)} total_bytes={total_bytes} nonconstant_checked={nonconstant}/{checked}"
)
PY
