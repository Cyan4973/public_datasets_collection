...
DATASET_ID = "uci_letter_recognition_u8"
PRIMARY_SERIES = {"letter_ocr_features"}
...
def inspect_u8(path: Path, series_id: str) -> tuple[int, int]:
    size = 0
    distinct = set()
    with path.open("rb") as fh:
        for chunk in iter(lambda: fh.read(1 << 20), b""):
            ...
            if series_id == "letter_ocr_features" and any(value > 15 for value in chunk):
                raise SystemExit("feature value outside 0..15")
            if series_id == "letter_labels_ascii" and any(value < ord("A") or value > ord("Z") for value in chunk):
                raise SystemExit("label byte outside A..Z")
...
for row in rows:
    ...
    size, distinct = inspect_u8(path, row["series_id"])
