...
expected = {
    "openfda_device_event_date_received_ymd_u32": ("uint", 32, 4, "<I"),
    "openfda_device_event_date_changed_ymd_u32": ("uint", 32, 4, "<I"),
    "openfda_device_event_mdr_text_count_u16": ("uint", 16, 2, "<H"),
    "openfda_device_event_patient_problem_count_u16": ("uint", 16, 2, "<H"),
    "openfda_device_event_product_problem_count_u16": ("uint", 16, 2, "<H"),
}
...
for row in primary:
    ...
    if row.get("sample_geometry") != "openfda_device_event_column" or int(row.get("sample_rank", 0)) != 1:
...
print(f"verified samples={len(primary)} retained_records={retained} total_values={sum(counts)} total_bytes={sum(byte_counts)}")
PY
