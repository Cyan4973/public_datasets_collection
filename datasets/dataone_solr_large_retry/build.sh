...
DATASET_ID="dataone_solr_large_retry"
...
src=json.load((download_dir/"dataone_solr_large_retry.json").open(encoding="utf-8"))
...
for row in src["response"]["docs"]:
    vals={
        "dataone_size": row.get("size"),
        "dataone_number_replicas": row.get("numberReplicas"),
    ...
    for sid,val in vals.items():
        try:
            if sid == "dataone_size":
                ...
                series[sid].append(int(val))
            elif sid == "dataone_date_modified":
                series[sid].append(ts64(val))
            else:
...
for sid,values in series.items():
    ...
    rows.append({"dataset_id":"dataone_solr_large_retry","series_id":sid,"sample_path":out.relative_to(data_root).as_posix(),"numeric_kind":kind,"bit_width":bits,"endianness":"little","element_size_bytes":bits//8,"sample_size_bytes":out.stat().st_size,"value_count":len(values)})
(filter_dir/"ingest_stats.json").write_text(json.dumps({"dataset_id":"dataone_solr_large_retry","rows_total":len(src["response"]["docs"]),"rows_skipped":skipped},indent=2,sort_keys=True)+"\n",encoding="utf-8")
