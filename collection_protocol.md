...
- Homogeneous family: a set of recipes or series that belong together because they share the same material type, generation process, cadence, and unit semantics. Sharing only the same API, vendor, or publication portal is not enough to make a homogeneous family.
...
- When replacing many thin recipes with a larger family recipe, the replacement must remain materially homogeneous. It is acceptable to combine closely related series such as multiple currency pairs with the same cadence and semantics, or multiple observations of the same physical quantity across stations. It is not acceptable to combine unrelated indicators merely because they come from the same API or portal.
- Homogeneous consolidation requires all of the following:
  - same source family or tightly related upstream publication surface
  - same material type
  - same or directly comparable cadence
  - same or directly comparable unit semantics
  - interpretable as one coherent dataset rather than a grab-bag of unrelated indicators
- If a fragmented source family cannot be consolidated without violating homogeneity, the correct action is to remove thin standalones or replace them with a smaller number of homogeneous bundles, not one oversized mixed bundle.
