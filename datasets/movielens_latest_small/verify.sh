...
for row in rows:
    ...
    if not p.is_file(): raise SystemExit(f"missing sample {p}")
    if p.stat().st_size != row["sample_size_bytes"]: raise SystemExit(f"size mismatch {p}")
print(f"verified_samples={len(rows)} rows_total={stats['rows_total']}")
PY
