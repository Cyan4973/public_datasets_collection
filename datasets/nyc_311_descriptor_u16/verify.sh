...
for required in [csv_path, stats_path, index_path]:
...
series_stats = stats["series"]["nyc_311_descriptor_id"]
if sum(row["value_count"] for row in rows) != series_stats["values"]:
    raise SystemExit("value count mismatch between index and stats")

for row in rows:
    if row["dataset_id"] != "nyc_311_descriptor_u16" or row["series_id"] != "nyc_311_descriptor_id":
