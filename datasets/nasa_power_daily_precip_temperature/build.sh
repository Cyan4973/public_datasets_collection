...
series = {"series_id": "nasa_power_precip_prectotcorr_f64", "array_type": "d", "numeric_kind": "float", "bit_width": 64, "endianness": "little", "element_size_bytes": 8}
...
for location_id in sorted(group_values):
    payload = array.array(series["array_type"], group_values[location_id])
    if payload.itemsize > 1 and os.sys.byteorder != "little":
        payload.byteswap()
    out_path = series_dir / f"{location_id}.bin"
    ...
    index_records.append({
        ...
        "value_count": len(group_values[location_id]),
