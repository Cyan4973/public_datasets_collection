...
TOK_OUT="$DOWNLOAD_DIR/tokenizer.json"
if [ -s "$TOK_OUT" ]; then
  tok_bytes="$(wc -c < "$TOK_OUT" | tr -d ' ')"
  echo "cached tokenizer bytes=$tok_bytes"
else
  curl -fL --retry 3 --retry-delay 2 -o "$TOK_OUT" \
...
if len(vocab) < 30000:
    raise SystemExit("unexpectedly small tokenizer vocab")
print(f"validated_tokenizer_vocab={len(vocab)}")
PY
