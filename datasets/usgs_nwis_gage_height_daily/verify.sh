...
DOWNLOAD_ROOT="${DATA_DIR}/downloads/usgs_nwis_gage_height_daily"
FILTERED_ROOT="${DATA_DIR}/filtered/usgs_nwis_gage_height_daily"
INDEX_ROOT="${DATA_DIR}/index/usgs_nwis_gage_height_daily"
SAMPLES_ROOT="${DATA_DIR}/samples/usgs_nwis_gage_height_daily"
LOG_ROOT="${DATA_DIR}/logs/usgs_nwis_gage_height_daily"
...
series_defs = [
    {"series_id": "usgs_gage_height_ft_f64", "numeric_kind": "float", "bit_width": 64, "endianness": "little", "element_size_bytes": 8},
...
stats_by_key = {(row["site_id"], row["year"]): row for row in stats_rows}
...
for site_id in site_ids:
    ...
    for year in range(2021, 2024):
        ...
        if json_path.stat().st_size <= 0:
            raise SystemExit(f"empty raw JSON: {json_path}")
        ...
        if stats_row is None:
            raise SystemExit(f"missing stats row for {site_id} {year}")
        if int(stats_row["row_count"]) != row_count:
            raise SystemExit(
                f"row count mismatch for {site_id} {year}: stats={stats_row['row_count']} raw={row_count}"
            )
        if int(stats_row["value_count"]) != value_count:
            raise SystemExit(
                f"value count mismatch for {site_id} {year}: stats={stats_row['value_count']} raw={value_count}"
            )
        if int(stats_row["skipped_count"]) != skipped_count:
            raise SystemExit(
                f"skipped count mismatch for {site_id} {year}: stats={stats_row['skipped_count']} raw={skipped_count}"
            )
        if stats_row["start_date"] != first_date:
            raise SystemExit(
                f"start date mismatch for {site_id} {year}: stats={stats_row['start_date']!r} raw={first_date!r}"
            )
        if stats_row["end_date"] != last_date:
            raise SystemExit(
                f"end date mismatch for {site_id} {year}: stats={stats_row['end_date']!r} raw={last_date!r}"
            )
