...
DATASET_ID="openalex_institutions_large"
...
page_re = re.compile(r"institutions_page_(\d+)\.json$")
page_paths = sorted(
    [p for p in page_dir.glob("institutions_page_*.json") if page_re.search(p.name)],
...
meta = {
    "openalex_inst_works_count": ("uint", 32, "I"),
    "openalex_inst_cited_by_count": ("uint", 32, "I"),
...
for path in page_paths:
    ...
    for item in obj["results"]:
        rows_total += 1
        before = len(vals["openalex_inst_works_count"])
        try:
            entity_id = item["id"]
            if entity_id in seen_ids:
                raise ValueError(f"duplicate OpenAlex institution id {entity_id}")
            seen_ids.add(entity_id)
            stats = item.get("summary_stats") or {}
            vals["openalex_inst_works_count"].append(as_uint(item["works_count"], "works_count", 0xFFFFFFFF))
            vals["openalex_inst_cited_by_count"].append(as_uint(item["cited_by_count"], "cited_by_count", 0xFFFFFFFF))
            vals["openalex_inst_h_index"].append(as_uint(stats.get("h_index") or 0, "h_index", 0xFFFFFFFF))
            vals["openalex_inst_i10_index"].append(as_uint(stats.get("i10_index") or 0, "i10_index", 0xFFFFFFFF))
            vals["openalex_inst_repo_count"].append(count_array(item, "repositories", 0xFFFF))
            vals["openalex_inst_role_count"].append(count_array(item, "roles", 255))
            vals["openalex_inst_assoc_count"].append(count_array(item, "associated_institutions", 0xFFFF))
        except Exception:
...
kept_rows = len(vals["openalex_inst_works_count"])
    ...
    rows.append(
        {
            ...
            "natural_record_kind": "openalex_institution_row",
