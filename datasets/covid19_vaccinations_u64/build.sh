...
src=dl/"vaccinations.csv"
fields=["new_persons_vaccinated","cumulative_persons_vaccinated","new_persons_fully_vaccinated","cumulative_persons_fully_vaccinated","new_vaccine_doses_administered","cumulative_vaccine_doses_administered"]
...
with src.open('r',encoding='utf-8',errors='replace',newline='') as fh:
    ...
    for row in reader:
        for k in fields:
            ...
            if v<0 or v>10_000_000_000: continue
            buffers[k].append(v)
index_rows=[]
total_bytes=0
for k, vals in buffers.items():
    ...
    total_bytes+=size
    index_rows.append({
        "dataset_id":"covid19_vaccinations_u64",
        "series_id":series_id,
        "role":"primary",
        ...
        "sample_format":"raw homogeneous uint64 vaccination count column",
        "sample_geometry":"table_column",
        "sample_rank":1,
        "sample_shape":[len(vals)],
        "sample_axes":["record"],
        "natural_record_kind":"covid19_vaccination_count_column",
...
(filt/"ingest_stats.json").write_text(json.dumps({"dataset_id":"covid19_vaccinations_u64","primary_samples":len(index_rows),"primary_values":pv,"primary_bytes":pb,"median_values":med},indent=2,sort_keys=True)+"\n")
