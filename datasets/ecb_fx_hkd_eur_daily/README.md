# ECB FX HKD/EUR Daily

This recipe collects the ECB daily HKD/EUR exchange-rate series and emits one
sample per year.

Selected scope:
- ECB series key: `D.HKD.EUR.SP00.A`
...
```sh
bash datasets/ecb_fx_hkd_eur_daily/download.sh
bash datasets/ecb_fx_hkd_eur_daily/build.sh
bash datasets/ecb_fx_hkd_eur_daily/verify.sh
```
