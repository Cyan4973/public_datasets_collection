...
facts = payload.get("facts", {}).get("us-gaap", {}).get("OperatingIncomeLoss", {}).get("units", {}).get("USD")
if not isinstance(facts, list) or not facts:
    raise SystemExit(f"missing OperatingIncomeLoss USD facts in {path}")
if not any(isinstance(row, dict) and row.get("fy") and row.get("fp") and row.get("val") not in ("", None) for row in facts):
    raise SystemExit(f"no usable quarterly OperatingIncomeLoss facts in {path}")
PY
}
