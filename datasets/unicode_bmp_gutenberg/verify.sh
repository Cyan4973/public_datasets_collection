...
texts = sorted(download_dir.glob("*.txt"))
if len(texts) != 12:
    raise SystemExit(f"expected 12 source texts, found {len(texts)}")
...
if len(rows) != 12:
    raise SystemExit(f"unexpected index row count: {len(rows)} != 12")

for row in rows:
    if row["dataset_id"] != "unicode_bmp_gutenberg":
        raise SystemExit(f"unexpected dataset id in row: {row}")
    if row["series_id"] != "unicode_bmp_codeunits":
        raise SystemExit(f"unexpected series id in row: {row}")
    if row["bit_width"] != 16 or row["numeric_kind"] != "uint" or row["element_size_bytes"] != 2 or row["endianness"] != "little":
    ...
    if row["sample_size_bytes"] != row["value_count"] * 2:
        raise SystemExit(f"bad element accounting: {row}")

if stats["series"]["unicode_bmp_codeunits"]["files"] != 12:
