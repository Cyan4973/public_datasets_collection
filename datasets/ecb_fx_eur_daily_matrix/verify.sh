...
stats_by_key = {(row["pair"], row["year"]): row for row in stats_rows}
expected_records = {}
...
for series in series_defs:
    ...
    for year in sorted(y for y in rows_by_year if y.isdigit()):
        ...
        if parsed:
            start_date = parsed[0]
            end_date = parsed[-1]
        stats_row = stats_by_key.get((series["pair"], year))
        if stats_row is None:
            raise SystemExit(f"missing stats row for {(series['pair'], year)}")
        for field, value in [("row_count", row_count), ("kept_count", kept_count), ("skipped_blank_count", skipped_blank_count), ("skipped_parse_count", skipped_parse_count)]:
            if int(stats_row[field]) != value:
                raise SystemExit(f"stats mismatch for {(series['pair'], year)} field {field}: stats={stats_row[field]} raw={value}")
        if stats_row["start_date"] != start_date or stats_row["end_date"] != end_date:
            raise SystemExit(f"date-range mismatch for {(series['pair'], year)}")
