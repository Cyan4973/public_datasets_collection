...
OUT="$DOWNLOAD_DIR/nuget_search.json"
TMP="$OUT.tmp"
URL="https://azuresearch-usnc.nuget.org/query?q=data&prerelease=false&take=100"
...
if not (isinstance(obj, dict) and isinstance(obj.get('data'), list)):
    raise SystemExit('bad nuget_search payload')
PY
  mv "$TMP" "$OUT"
fi
