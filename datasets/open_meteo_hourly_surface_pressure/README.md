# Open-Meteo Hourly Surface_Pressure
...
- `download.sh` validates that each JSON payload contains `hourly.time` and
  `hourly.surface_pressure` with matching lengths before accepting it into cache.
...
```sh
bash datasets/open_meteo_hourly_surface_pressure/download.sh
bash datasets/open_meteo_hourly_surface_pressure/build.sh
bash datasets/open_meteo_hourly_surface_pressure/verify.sh
```
