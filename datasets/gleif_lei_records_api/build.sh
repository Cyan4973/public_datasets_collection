...
rows=json.load(open(download_dir/'records.json', encoding='utf-8'))['data']
...
for row in rows:
    ...
    try:
        creation=datetime.fromisoformat(entity['creationDate'].replace('Z','+00:00')).year
        initial=datetime.fromisoformat(reg['initialRegistrationDate'].replace('Z','+00:00')).year
        renewal=datetime.fromisoformat(reg['nextRenewalDate'].replace('Z','+00:00')).year
    except Exception:
        skipped += 1
        continue
...
for s in series_defs:
    ...
    out=samples_dir/s['series_id']/'records.bin'
    with out.open('wb') as fh: fh.write(arr.tobytes())
    records.append({'dataset_id':'gleif_lei_records_api','series_id':s['series_id'],'sample_path':out.relative_to(data_root).as_posix(),'numeric_kind':s['numeric_kind'],'bit_width':s['bit_width'],'endianness':s['endianness'],'element_size_bytes':s['element_size_bytes'],'sample_size_bytes':out.stat().st_size,'value_count':len(vals[s['series_id']])})
