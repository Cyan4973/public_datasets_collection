...
OUT="$DOWNLOAD_DIR/geofabrik_index.json"
TMP="$OUT.tmp"
...
if ! curl --globoff -fL --retry 2 --retry-delay 2 -A "openzl-public-datasets/1.0" -o "$TMP" "$URL"; then
  echo -e "geofabrik_index\tcurl_failed\t$URL" >> "$FAIL_FILE"
  rm -f "$TMP"
  exit 1
fi
python3 - <<'PY' "$TMP"
...
assert isinstance(obj, dict) and 'features' in obj and len(obj['features']) > 0
PY
mv "$TMP" "$OUT"
echo "[$(date -Is)] download done dataset=$DATASET_ID"
