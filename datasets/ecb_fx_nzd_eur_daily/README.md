...
This recipe collects the ECB daily USD/EUR exchange-rate series and emits one
sample per year.

Selected scope:
- ECB series key: `D.NZD.EUR.SP00.A`
...
```sh
bash datasets/ecb_fx_nzd_eur_daily/download.sh
bash datasets/ecb_fx_nzd_eur_daily/build.sh
bash datasets/ecb_fx_nzd_eur_daily/verify.sh
```
