...
wind observations and converts them into raw numeric samples.
...
- `download.sh` validates that each JSON payload contains `hourly.time` and
  `hourly.wind_speed_10m` with matching lengths before accepting it into cache.
...
```sh
bash datasets/open_meteo_hourly_wind/download.sh
bash datasets/open_meteo_hourly_wind/build.sh
bash datasets/open_meteo_hourly_wind/verify.sh
```
