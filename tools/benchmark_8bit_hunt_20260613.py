...
def expected_downloads(manifest: dict, data_root: Path) -> list[Path]:
    ...
    for resource in manifest.get("resources", []):
        ...
        if local_path:
            paths.append(data_root / str(local_path))
    return paths


def require_downloads(recipe_dir: Path, data_root: Path) -> None:
    missing = [path for path in expected_downloads(load_manifest(recipe_dir), data_root) if not path.exists()]
    if missing:
        rels = "\n".join(f"- {path.relative_to(REPO_ROOT)}" for path in missing)
        raise SystemExit(
            f"missing local download(s) for {recipe_dir.name}; run its download.sh or rerun this script with --download:\n{rels}"
        )
...
def summarize_dataset(recipe_dir: Path, data_root: Path) -> dict:
    ...
    for line in index_path.read_text(encoding="utf-8").splitlines():
        ...
        primary_sizes.append(declared_size)
        primary_values.append(declared_values)

    if not primary_sizes:
        raise SystemExit(f"{dataset_id}: no primary samples in index")
    size_counts = Counter(primary_sizes)
    total_bytes = sum(primary_sizes)
    total_values = sum(primary_values)
    reasons = []
    if missing_files:
    ...
    if statistics.median(primary_values) < MIN_MEDIAN_VALUES:
    ...
    return {
        ...
        "size_min": min(primary_sizes),
        "size_p10": percentile(primary_sizes, 0.10),
        "size_p25": percentile(primary_sizes, 0.25),
        "size_median": statistics.median(primary_sizes),
        "size_p75": percentile(primary_sizes, 0.75),
        "size_p90": percentile(primary_sizes, 0.90),
        "size_max": max(primary_sizes),
        "unique_sample_sizes": len(size_counts),
        "same_size_fraction": max(size_counts.values()) / len(primary_sizes),
        "missing_files": missing_files,
    }


def write_benchmark_reports(summaries: list[dict], output_json: Path, output_tsv: Path) -> None:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run the 2026-06-13 staged 8-bit benchmark locally.")
    parser.add_argument("--data-dir", default=DEFAULT_DATA_DIR)
    parser.add_argument("--download", action="store_true", help="Run each dataset download.sh before build/verify. Default requires local downloads to already exist.")
    parser.add_argument("--skip-build", action="store_true", help="Only summarize existing indexes and samples.")
    parser.add_argument("--output-json", default="reports/8bit_hunt_20260613_benchmark.json")
    parser.add_argument("--output-tsv", default="reports/8bit_hunt_20260613_benchmark.tsv")
    parser.add_argument("--state-md", default="reports/8bit_hunt_20260613_dataset_state.md")
    parser.add_argument("--state-tsv", default="reports/8bit_hunt_20260613_dataset_state.tsv")
    args = parser.parse_args()

    data_root = REPO_ROOT / args.data_dir
    recipe_dirs = [REPO_ROOT / "staging" / dataset_id for dataset_id in DATASET_IDS]
    ...
    run(
        [
            ...
            *(str(recipe.relative_to(REPO_ROOT)) for recipe in recipe_dirs),
            "--data-dir",
            args.data_dir,
            "--output-md",
            args.state_md,
    ...
    summaries = [summarize_dataset(recipe_dir, data_root) for recipe_dir in recipe_dirs]
    write_benchmark_reports(summaries, REPO_ROOT / args.output_json, REPO_ROOT / args.output_tsv)
    for summary in summaries:
        print(
