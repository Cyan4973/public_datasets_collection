...
EXPECTED_PRIMARY = {"midi_note_pitch_u8", "midi_note_velocity_u8"}
...
for r in primary:
    ...
    if max(data) > 127:  # MIDI pitch/velocity are 7-bit
        raise SystemExit(f"value out of MIDI range in {r['sample_path']}")
    if len(set(data)) <= 1:
