...
TGT="$DOWNLOAD_DIR/hospitalizations.csv"
echo "fetch $URL"
...
dd=repo/Path(os.environ.get("DATA_DIR",".data"))/"downloads"/"covid19_hospitalizations_u64"
if not dd.exists():
    dd=Path(".data/downloads/covid19_hospitalizations_u64")
f=dd/"hospitalizations.csv"
assert f.exists() and f.stat().st_size>0
print(f"bytes={f.stat().st_size}")
txt=f.read_text(errors='replace')[:5000]
assert "hospitalized" in txt.lower()
(dd/"download_inventory.json").write_text(json.dumps({"dataset_id":"covid19_hospitalizations_u64","bytes":f.stat().st_size},indent=2,sort_keys=True)+"\n")
