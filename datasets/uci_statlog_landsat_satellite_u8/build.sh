...
archive = download_dir / "statlog-landsat-satellite.zip"
...
features_dir = samples_dir / "landsat_spectral_features"
labels_dir = samples_dir / "landsat_class_labels"
reset_dir(features_dir)
reset_dir(labels_dir)
filter_dir.mkdir(parents=True, exist_ok=True)
index_dir.mkdir(parents=True, exist_ok=True)

expected_rows = [("train", "sat.trn", 4435), ("test", "sat.tst", 2000)]
stats = {"dataset_id": DATASET_ID, "splits": []}
sample_rows = []
with zipfile.ZipFile(archive) as zf:
    base_to_name = {Path(name).name: name for name in zf.namelist() if not name.endswith("/")}
    for split, base, expected_count in expected_rows:
        ...
        with zf.open(member) as raw:
            for line_number, raw_line in enumerate(raw, start=1):
                ...
                if len(values) != 37:
                    raise RuntimeError(f"{base}:{line_number}: expected 37 columns, got {len(values)}")
                for value in values[:36]:
                    if value < 0 or value > 255:
                        raise RuntimeError(f"{base}:{line_number}: feature outside uint8 range")
                    feature_payload.append(value)
                    feature_counts[value] += 1
                label = values[36]
                if label < 1 or label > 7:
                    raise RuntimeError(f"{base}:{line_number}: label outside 1..7")
        ...
        if len(feature_payload) != rows * 36:
        ...
        feature_out = features_dir / f"{split}_landsat_spectral_features_u8.bin"
        label_out = labels_dir / f"{split}_landsat_class_labels_u8.bin"
        ...
        sample_rows.append({
            ...
            "series_id": "landsat_spectral_features",
        ...
        sample_rows.append({
            "dataset_id": DATASET_ID,
            "series_id": "landsat_class_labels",
