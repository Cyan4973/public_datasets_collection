...
DOWNLOAD_ROOT="${DATA_DIR}/downloads/nasa_power_daily_precip_solar"
FILTERED_ROOT="${DATA_DIR}/filtered/nasa_power_daily_precip_solar"
INDEX_ROOT="${DATA_DIR}/index/nasa_power_daily_precip_solar"
SAMPLES_ROOT="${DATA_DIR}/samples/nasa_power_daily_precip_solar"
LOG_ROOT="${DATA_DIR}/logs/nasa_power_daily_precip_solar"
...
parameter_ids = ["PRECTOTCORR", "ALLSKY_SFC_SW_DWN", "RH2M"]
series_defs = [
...
for (location_id, parameter_id), value_count in sorted(group_counts.items()):
    ...
    for series in series_defs:
        ...
        expected_records[(series["series_id"], sample_slug)] = {
            "dataset_id": "nasa_power_daily_precip_solar",
