...
ID="covid19_hospitalizations_u64"
...
src=dl/"hospitalizations.csv"
if not src.exists():
    raise SystemExit("missing hospitalizations.csv")
fields=["new_hospitalized_patients","cumulative_hospitalized_patients","current_hospitalized_patients","new_intensive_care_patients","cumulative_intensive_care_patients","current_intensive_care_patients","new_ventilator_patients","cumulative_ventilator_patients","current_ventilator_patients"]
...
for k, vals in buffers.items():
    if len(vals)<1000 or len(set(vals))<=1:
        print(f"skip {k} count={len(vals)} unique={len(set(vals))}")
        continue
    series_id=f"covid19_{k}_u64"
    ...
    total_bytes+=size
    index_rows.append({
        "dataset_id":"covid19_hospitalizations_u64",
        "series_id":series_id,
        "role":"primary",
        ...
        "sample_format":"raw homogeneous uint64 hospitalization count column",
        "sample_geometry":"table_column",
        "sample_rank":1,
        "sample_shape":[len(vals)],
        "sample_axes":["record"],
        "natural_record_kind":"covid19_hospitalization_count_column",
...
if pb>1_000_000_000:
    raise SystemExit(f"exceeds cap {pb}")
with (idx/"samples.jsonl").open("w") as fh:
    for r in sorted(index_rows,key=lambda x:x["sample_path"]):
        fh.write(json.dumps(r,sort_keys=True)+"\n")
(filt/"ingest_stats.json").write_text(json.dumps({"dataset_id":"covid19_hospitalizations_u64","primary_samples":len(index_rows),"primary_values":pv,"primary_bytes":pb,"median_values":med},indent=2,sort_keys=True)+"\n")
