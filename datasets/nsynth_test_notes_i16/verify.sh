...
for row in rows:
    ...
    if row.get("sample_format") != "raw homogeneous int16 PCM array" or row.get("natural_record_kind") != "nsynth_note_wav_file":
        raise SystemExit(f"missing or invalid sample semantics: {row}")
    if row.get("min") == row.get("max"):
        raise SystemExit(f"constant note sample metadata: {path}")
