...
MIN_RECORDS="${OPENALEX_MIN_RECORDS:-80000}"
...
while [ "$rows_downloaded" -lt "$MAX_RECORDS" ]; do
  out="$PAGE_DIR/institutions_page_$(printf '%04d' "$page").json"
  tmp="$out.tmp"
  if [ -s "$out" ] && [ "${FORCE_DOWNLOAD:-0}" != "1" ]; then
    ...
    CURL_ARGS=(
      ...
      -o "$tmp"
      --data-urlencode "select=id,works_count,cited_by_count,summary_stats,repositories,roles,associated_institutions"
...
rows_total = 0
api_total = None
for path in sorted(page_dir.glob("institutions_page_*.json")):
