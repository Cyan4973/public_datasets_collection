...
DOWNLOAD_ROOT="${DATA_DIR}/downloads/noaa_ghcn_daily_by_station"
FILTERED_ROOT="${DATA_DIR}/filtered/noaa_ghcn_daily_by_station"
INDEX_ROOT="${DATA_DIR}/index/noaa_ghcn_daily_by_station"
SAMPLES_ROOT="${DATA_DIR}/samples/noaa_ghcn_daily_by_station"
LOG_ROOT="${DATA_DIR}/logs/noaa_ghcn_daily_by_station"
...
element_ids = ["TMAX", "TMIN", "PRCP", "SNOW", "SNWD"]
series_defs = [
    ...
    for station_id in station_ids:
        ...
        with gzip.open(csv_path, "rt", encoding="utf-8", newline="") as handle:
            ...
            for row in reader:
                if len(row) < 8:
                    continue
                row_station, raw_date, element_id, raw_value, _mflag, qflag, _sflag, _obs_time = row[:8]
                if row_station != station_id or element_id not in element_ids:
                    continue
...
for key in sorted(group_values):
    ...
    payloads = {
        "ghcn_value_i16": group_values[key],
