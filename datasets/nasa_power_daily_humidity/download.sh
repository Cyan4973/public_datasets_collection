...
plan_path = Path(sys.argv[1]); base_url = "https://power.larc.nasa.gov/api/temporal/daily/point"
locations = [("san_francisco","37.7749","-122.4194"),("phoenix","33.4484","-112.0740"),("chicago","41.8781","-87.6298"),("miami","25.7617","-80.1918"),("anchorage","61.2181","-149.9003")]
with plan_path.open("w", encoding="utf-8", newline="") as plan_file:
 for location_id, lat, lon in locations:
  for year in range(2021, 2024):
   params = {"parameters":"RH2M,T2MDEW,ALLSKY_SFC_SW_DWN","community":"RE","longitude":lon,"latitude":lat,"start":f"{year}0101","end":f"{year}1231","format":"JSON","time-standard":"UTC"}
...
fetch() {
 ...
 if [ -f "${out}" ] && [ "${FORCE}" != "1" ]; then return 2; fi
 rm -f "${tmp}"
 if command -v curl >/dev/null 2>&1; then curl -fL --retry 3 --retry-delay 5 -o "${tmp}" "${url}" >>"${LOG_FILE}" 2>&1
...
while IFS='	' read -r location year lat lon url rel_out; do
 [ -n "${location}" ] || continue
 out="${DOWNLOAD_ROOT}/${rel_out}"; say "fetch ${location} ${year} ${url}"
 if fetch "${url}" "${out}"; then success_count=$((success_count + 1)); say "ok ${location} ${year} ${out}"
 else status=$?; if [ "${status}" -eq 2 ]; then cached_count=$((cached_count + 1)); say "cached ${location} ${year} ${out}"
 else failure_count=$((failure_count + 1)); rm -f "${out}" "${out}.tmp"; printf '%s\t%s\t%s\t%s\t%s\t%s\n' "${location}" "${year}" "${lat}" "${lon}" "${url}" "${rel_out}" >> "${FAILURES_FILE}"; say "failed ${location} ${year} ${url}"; fi; fi
done < "${PLAN_FILE}"
