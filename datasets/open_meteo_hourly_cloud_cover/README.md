...
- `download.sh` validates that each JSON payload contains `hourly.time` and
  `hourly.cloud_cover` with matching lengths before accepting it into cache.
...
```sh
bash datasets/open_meteo_hourly_cloud_cover/download.sh
bash datasets/open_meteo_hourly_cloud_cover/build.sh
bash datasets/open_meteo_hourly_cloud_cover/verify.sh
```
