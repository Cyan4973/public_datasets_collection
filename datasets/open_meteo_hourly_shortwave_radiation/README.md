# Open-Meteo Hourly shortwave radiation
...
- `download.sh` validates that each JSON payload contains `hourly.time` and
  `hourly.shortwave_radiation` with matching lengths before accepting it into cache.
...
```sh
bash datasets/open_meteo_hourly_shortwave_radiation/download.sh
bash datasets/open_meteo_hourly_shortwave_radiation/build.sh
bash datasets/open_meteo_hourly_shortwave_radiation/verify.sh
```
