...
for row in primary:
    ...
    if row.get("sample_geometry") != "table_column" or row.get("natural_record_kind") != "uci_steel_plate_fault_feature_column":
        raise SystemExit(f"unexpected sample metadata: {row}")
    if int(row["source_column_index"]) >= 27:
        raise SystemExit(f"class label column was emitted: {row}")
