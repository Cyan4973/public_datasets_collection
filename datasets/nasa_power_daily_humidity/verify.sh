...
DOWNLOAD_ROOT="${DATA_DIR}/downloads/nasa_power_daily_humidity"
FILTERED_ROOT="${DATA_DIR}/filtered/nasa_power_daily_humidity"
INDEX_ROOT="${DATA_DIR}/index/nasa_power_daily_humidity"
SAMPLES_ROOT="${DATA_DIR}/samples/nasa_power_daily_humidity"
LOG_ROOT="${DATA_DIR}/logs/nasa_power_daily_humidity"
...
stats_path = filtered_root / "location_parameter_year_stats.tsv"; index_path = index_root / "samples.jsonl"; failures_path = download_root / "download_failures.tsv"
...
expected_records = {}; group_counts = defaultdict(int)
for location_id in locations:
 for year in range(2021, 2024):
  path = download_root / f"{location_id}_{year}.json"
  if not path.is_file(): raise SystemExit(f"missing raw JSON: {path}")
  if path.stat().st_size <= 0: raise SystemExit(f"empty raw JSON: {path}")
  payload = json.loads(path.read_text(encoding="utf-8"))
  header = payload.get("header", {}); fill_value = header.get("fill_value"); parameter_block = payload.get("properties", {}).get("parameter", {})
  if not isinstance(parameter_block, dict): raise SystemExit(f"unexpected parameter payload in {path}")
  for parameter_id in parameter_ids:
   values = parameter_block.get(parameter_id)
   if not isinstance(values, dict): raise SystemExit(f"missing parameter {parameter_id} in {path}")
   row_count = len(values); kept_count = skipped_fill_count = skipped_parse_count = 0; start_date = end_date = ""
   for date_key in sorted(values):
    raw_value = values[date_key]
    if len(date_key) != 8 or not date_key.isdigit(): skipped_parse_count += 1; continue
    if raw_value == fill_value: skipped_fill_count += 1; continue
    if isinstance(raw_value, str) and raw_value.strip().upper() in {"", "NAN"}: skipped_parse_count += 1; continue
    try: float(raw_value); obs_month = int(date_key[4:6]); obs_day = int(date_key[6:8])
    except (TypeError, ValueError): skipped_parse_count += 1; continue
    if obs_month < 1 or obs_month > 12 or obs_day < 1 or obs_day > 31: skipped_parse_count += 1; continue
    kept_count += 1; group_counts[(location_id, parameter_id)] += 1
    if start_date == "": start_date = date_key
    end_date = date_key
   stats_row = stats_by_key.get((location_id, parameter_id, str(year)))
   if stats_row is None: raise SystemExit(f"missing stats row for {location_id} {parameter_id} {year}")
   for field, value in [("row_count", row_count), ("kept_count", kept_count), ("skipped_fill_count", skipped_fill_count), ("skipped_parse_count", skipped_parse_count)]:
    if int(stats_row[field]) != value: raise SystemExit(f"stats mismatch for {location_id} {parameter_id} {year} field {field}: stats={stats_row[field]} raw={value}")
   if stats_row["start_date"] != start_date: raise SystemExit(f"start date mismatch for {location_id} {parameter_id} {year}: stats={stats_row['start_date']!r} raw={start_date!r}")
   if stats_row["end_date"] != end_date: raise SystemExit(f"end date mismatch for {location_id} {parameter_id} {year}: stats={stats_row['end_date']!r} raw={end_date!r}")
for (location_id, parameter_id), value_count in sorted(group_counts.items()):
 ...
 for series in series_defs:
  ...
  sample_size_bytes = sample_path.stat().st_size; expected_size = value_count * int(series["element_size_bytes"])
