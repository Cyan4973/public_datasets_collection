...
if len(primary) != MIN_SAMPLES:
    raise SystemExit(f"expected {MIN_SAMPLES} primary samples, got {len(primary)}")
...
for row in primary:
    ...
    if row.get("sample_geometry") != "table_column" or row.get("natural_record_kind") != "magic_gamma_telescope_event_feature_column":
...
if len(fields) != MIN_SAMPLES:
    raise SystemExit(f"expected {MIN_SAMPLES} fields, got {len(fields)}")
