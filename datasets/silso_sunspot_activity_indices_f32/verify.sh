...
DATASET_ID = "silso_sunspot_activity_indices_f32"
MIN_SAMPLES = 6
MIN_PRIMARY_VALUES = 150_000
MIN_PRIMARY_BYTES = 200_000
MIN_MEDIAN_VALUES = 3_000
...
for row in primary:
    ...
    if row.get("sample_geometry") != "time_series" or row.get("natural_record_kind") != "silso_sunspot_activity_index_field":
    ...
    if any(value < 0 for value in values):
        raise SystemExit(f"negative sample values: {row['sample_path']}")
    if len(set(values)) <= 1:
    ...
    fields.add((row["source_cadence"], row["source_field"]))
