...
dataset_id = "nasa_power_daily_humidity"
locations = ["san_francisco","phoenix","chicago","miami","anchorage"]
parameter_ids = ["RH2M","T2MDEW","ALLSKY_SFC_SW_DWN"]
series_defs = [
...
group_values = defaultdict(list); group_years = defaultdict(list); group_months = defaultdict(list); group_days = defaultdict(list)
stats_path = filtered_root / "location_parameter_year_stats.tsv"; index_path = index_root / "samples.jsonl"; index_records = []
with stats_path.open("w", encoding="utf-8", newline="") as stats_file:
 writer = csv.writer(stats_file, delimiter="\t"); writer.writerow(["location_id","parameter_id","year","row_count","kept_count","skipped_fill_count","skipped_parse_count","start_date","end_date"])
  ...
  for year in range(2021, 2024):
   ...
   for parameter_id in parameter_ids:
    ...
    for date_key in sorted(values):
     ...
     try: value = float(raw_value); obs_year = int(date_key[:4]); obs_month = int(date_key[4:6]); obs_day = int(date_key[6:8])
     ...
     group_values[key].append(value); group_years[key].append(obs_year); group_months[key].append(obs_month); group_days[key].append(obs_day)
     kept_count += 1
