...
with plan_path.open("w", encoding="utf-8", newline="") as plan_file:
    for location_id, lat, lon in locations:
        for year in range(2022, 2024):
            ...
            plan_file.write(f"{location_id}\t{year}\t{lat}\t{lon}\t{parameter_id}\t{url}\t{out}\n")
PY
...
if not isinstance(hourly, dict):
    raise SystemExit(f"missing hourly block in {path}")
times = hourly.get("time")
values = hourly.get("vapour_pressure_deficit")
if not isinstance(times, list) or not times:
    raise SystemExit(f"missing hourly.time in {path}")
if not isinstance(values, list) or not values:
    raise SystemExit(f"missing hourly.vapour_pressure_deficit in {path}")
if len(times) != len(values):
    raise SystemExit(f"time/value length mismatch in {path}")
PY
}
...
cached_count=0
failure_count=0
while IFS='	' read -r location year lat lon parameter_id url rel_out; do
  [ -n "${location}" ] || continue
  ...
  if fetch "${url}" "${out}"; then
    ...
    if [ "${status}" -eq 2 ]; then
      ...
      rm -f "${out}" "${out}.tmp"
      printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "${location}" "${year}" "${lat}" "${lon}" "${parameter_id}" "${url}" "${rel_out}" >> "${FAILURES_FILE}"
