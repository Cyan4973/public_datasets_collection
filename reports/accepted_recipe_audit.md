# Accepted Recipe Audit

Acceptance floor: at least `10000` primary values total or at least `102400` primary sample bytes, plus median primary sample size at least `1000` values.

Auxiliary series do not count toward acceptance. When a manifest omits `role`, this audit currently infers `auxiliary` only for narrow calendar-helper names such as `obs_year_*`, `obs_month_*`, `obs_day_*`, and `obs_hour_*`.

- `ok`: 122
- `below_floor`: 209
- `broken`: 0

## Below Floor

| dataset_id | primary_values | primary_sample_bytes | primary_sample_rows | median_primary_sample_value_count | auxiliary_values | auxiliary_sample_rows | reasons |
|---|---:|---:|---:|---:|---:|---:|---|
| `fred_real_gdp_quarterly` | 40 | 160 | 10 | 4 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `rubygems_search` | 60 | 480 | 2 | 30 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `fred_capacity_utilization_monthly` | 120 | 480 | 10 | 12 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `fred_civilian_labor_force_monthly` | 120 | 480 | 10 | 12 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `fred_consumer_sentiment_monthly` | 120 | 480 | 10 | 12 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `fred_core_cpi_monthly` | 120 | 480 | 10 | 12 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `fred_cpi_all_items_monthly` | 120 | 480 | 10 | 12 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `fred_federal_funds_monthly` | 120 | 480 | 10 | 12 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `fred_housing_starts_monthly` | 120 | 480 | 10 | 12 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `fred_industrial_production_monthly` | 120 | 480 | 10 | 12 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `fred_labor_force_participation_monthly` | 120 | 480 | 10 | 12 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `fred_m2_money_stock_monthly` | 120 | 480 | 10 | 12 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `fred_payroll_employment_monthly` | 120 | 480 | 10 | 12 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `fred_pce_price_index_monthly` | 120 | 480 | 10 | 12 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `fred_ppi_all_commodities_monthly` | 120 | 480 | 10 | 12 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `fred_unemployment_level_monthly` | 120 | 480 | 10 | 12 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `fred_unemployment_rate_monthly` | 120 | 480 | 10 | 12 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `biorxiv_details` | 150 | 390 | 5 | 30 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `dryad_search` | 160 | 600 | 8 | 20 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `wikidata_sparql` | 200 | 600 | 2 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `maven_central_search` | 200 | 1000 | 2 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `internet_archive_metadata` | 200 | 1200 | 2 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `packagist_packages` | 200 | 1200 | 2 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `sec_companyfacts_assets_quarterly` | 227 | 1816 | 5 | 51 | 227 | 5 | `aggregate_floor,median_sample_floor` |
| `sec_companyfacts_cash_and_equivalents_quarterly` | 227 | 1816 | 5 | 51 | 227 | 5 | `aggregate_floor,median_sample_floor` |
| `sec_companyfacts_net_income_quarterly` | 227 | 1816 | 5 | 51 | 227 | 5 | `aggregate_floor,median_sample_floor` |
| `sec_companyfacts_operating_income_quarterly` | 227 | 1816 | 5 | 51 | 227 | 5 | `aggregate_floor,median_sample_floor` |
| `sec_companyfacts_stockholders_equity_quarterly` | 227 | 1816 | 5 | 51 | 227 | 5 | `aggregate_floor,median_sample_floor` |
| `openbrewerydb_breweries` | 246 | 1640 | 3 | 82 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `gutendex_books` | 256 | 576 | 8 | 32 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `nasa_neows_feed` | 270 | 1305 | 6 | 45 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `library_of_congress_items` | 276 | 1016 | 5 | 44 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `anilist_media` | 295 | 837 | 6 | 49 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `semanticscholar_papers` | 300 | 1000 | 3 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `dockerhub_repositories` | 300 | 1300 | 3 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `weathergov_stations` | 300 | 2000 | 3 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `openlibrary_subjects` | 305 | 692 | 6 | 41 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `world_bank_access_to_electricity_percent_annual` | 319 | 2552 | 10 | 34 | 319 | 10 | `aggregate_floor,median_sample_floor` |
| `world_bank_renewable_energy_consumption_percent_annual` | 320 | 2560 | 10 | 32 | 320 | 10 | `aggregate_floor,median_sample_floor` |
| `owid_consumption_co2_annual` | 340 | 1360 | 10 | 34 | 340 | 10 | `aggregate_floor,median_sample_floor` |
| `owid_trade_co2_annual` | 340 | 1360 | 10 | 34 | 340 | 10 | `aggregate_floor,median_sample_floor` |
| `owid_trade_co2_share_annual` | 340 | 1360 | 10 | 34 | 340 | 10 | `aggregate_floor,median_sample_floor` |
| `world_bank_forest_area_percent_annual` | 340 | 2720 | 10 | 34 | 340 | 10 | `aggregate_floor,median_sample_floor` |
| `world_bank_internet_users_percent_annual` | 344 | 2752 | 10 | 35 | 344 | 10 | `aggregate_floor,median_sample_floor` |
| `world_bank_energy_use_per_capita_annual` | 346 | 2768 | 10 | 35 | 346 | 10 | `aggregate_floor,median_sample_floor` |
| `world_bank_unemployment_total_percent_annual` | 350 | 2800 | 10 | 35 | 350 | 10 | `aggregate_floor,median_sample_floor` |
| `esco_occupations` | 355 | 639 | 5 | 71 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `orcid_search` | 390 | 780 | 4 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `openlibrary_search` | 400 | 800 | 5 | 80 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `europe_pmc_search` | 400 | 800 | 4 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `wger_exercises` | 400 | 800 | 4 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `arxiv_cs_recent` | 400 | 1200 | 4 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `osf_preprints` | 400 | 1300 | 4 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `stackexchange_top_questions_jan_2024` | 400 | 1400 | 4 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `hn_algolia_search` | 400 | 1600 | 4 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `cratesio_crates` | 400 | 2000 | 4 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `hex_packages` | 400 | 2600 | 4 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `osm_overpass_cafes` | 400 | 2600 | 4 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `imf_gdp_growth_annual` | 416 | 1664 | 8 | 52 | 416 | 8 | `aggregate_floor,median_sample_floor` |
| `imf_gdp_per_capita_usd_annual` | 416 | 1664 | 8 | 52 | 416 | 8 | `aggregate_floor,median_sample_floor` |
| `imf_general_government_gross_debt_gdp_annual` | 416 | 1664 | 8 | 52 | 416 | 8 | `aggregate_floor,median_sample_floor` |
| `imf_inflation_annual` | 416 | 1664 | 8 | 52 | 416 | 8 | `aggregate_floor,median_sample_floor` |
| `imf_nominal_gdp_usd_annual` | 416 | 1664 | 8 | 52 | 416 | 8 | `aggregate_floor,median_sample_floor` |
| `imf_population_annual` | 416 | 1664 | 8 | 52 | 416 | 8 | `aggregate_floor,median_sample_floor` |
| `imf_unemployment_rate_annual` | 416 | 1664 | 8 | 52 | 416 | 8 | `aggregate_floor,median_sample_floor` |
| `musicbrainz_recordings` | 476 | 1046 | 5 | 94 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `musicbrainz_release_groups` | 491 | 982 | 5 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `openfoodfacts_products` | 491 | 1870 | 5 | 98 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `inaturalist_observations` | 495 | 1188 | 5 | 99 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `openfda_drug_event` | 495 | 1683 | 5 | 99 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `pokemontcg_cards` | 498 | 1071 | 6 | 81 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `gbif_occurrence` | 498 | 2284 | 5 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `treasury_avg_interest_rates` | 500 | 1000 | 5 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `crossref_funders` | 500 | 1200 | 5 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `medrxiv_details` | 500 | 1300 | 5 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `artic_artworks_search` | 500 | 1400 | 5 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `figshare_articles` | 500 | 1400 | 5 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `nvd_cves_recent` | 500 | 1400 | 5 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `nuget_search` | 500 | 1500 | 5 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `europeana_search` | 500 | 2300 | 5 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `npm_search_packages` | 500 | 2600 | 5 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `fred_fed_balance_sheet_weekly` | 521 | 2084 | 10 | 52 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `fred_mortgage_30y_weekly` | 521 | 2084 | 10 | 52 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `fred_initial_claims_weekly` | 522 | 2088 | 10 | 52 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `owid_flaring_co2_annual` | 564 | 2256 | 10 | 57.5 | 564 | 10 | `aggregate_floor,median_sample_floor` |
| `owid_energy_per_gdp_annual` | 565 | 2260 | 10 | 58 | 565 | 10 | `aggregate_floor,median_sample_floor` |
| `world_bank_population_density_annual` | 567 | 4536 | 9 | 63 | 567 | 9 | `aggregate_floor,median_sample_floor` |
| `owid_energy_per_capita_annual` | 584 | 2336 | 10 | 60 | 584 | 10 | `aggregate_floor,median_sample_floor` |
| `owid_primary_energy_consumption_annual` | 584 | 2336 | 10 | 60 | 584 | 10 | `aggregate_floor,median_sample_floor` |
| `crossref_works_jan_2024` | 588 | 3136 | 3 | 196 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `gleif_lei_records` | 600 | 1200 | 6 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `ooni_measurements` | 600 | 1500 | 6 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `crossref_members` | 600 | 1800 | 6 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `datacite_dois` | 600 | 2000 | 6 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `deezer_chart` | 600 | 2000 | 6 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `openml_datasets` | 600 | 2000 | 6 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `openalex_works_2024_sample` | 600 | 2000 | 3 | 200 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `steamspy_top100in2weeks` | 600 | 2200 | 6 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `openml_dataset_61` | 600 | 2400 | 4 | 150 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `coingecko_top_markets` | 600 | 4200 | 6 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `world_bank_agricultural_land_percent_annual` | 630 | 5040 | 10 | 63 | 630 | 10 | `aggregate_floor,median_sample_floor` |
| `world_bank_birth_rate_annual` | 650 | 5200 | 10 | 65 | 650 | 10 | `aggregate_floor,median_sample_floor` |
| `world_bank_death_rate_annual` | 650 | 5200 | 10 | 65 | 650 | 10 | `aggregate_floor,median_sample_floor` |
| `world_bank_fertility_rate_annual` | 650 | 5200 | 10 | 65 | 650 | 10 | `aggregate_floor,median_sample_floor` |
| `world_bank_gdp_current_usd` | 650 | 5200 | 10 | 65 | 650 | 10 | `aggregate_floor,median_sample_floor` |
| `world_bank_gdp_per_capita_current_usd_annual` | 650 | 5200 | 10 | 65 | 650 | 10 | `aggregate_floor,median_sample_floor` |
| `world_bank_life_expectancy` | 650 | 5200 | 10 | 65 | 650 | 10 | `aggregate_floor,median_sample_floor` |
| `world_bank_population_total` | 650 | 5200 | 10 | 65 | 650 | 10 | `aggregate_floor,median_sample_floor` |
| `world_bank_urban_population_percent_annual` | 650 | 5200 | 10 | 65 | 650 | 10 | `aggregate_floor,median_sample_floor` |
| `itunes_search` | 697 | 3390 | 7 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `doaj_articles` | 700 | 1300 | 7 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `huggingface_datasets` | 700 | 2200 | 7 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `gitlab_projects` | 700 | 2800 | 7 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `marine_regions_gazetteer` | 758 | 5264 | 8 | 93.5 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `launchlibrary_upcoming` | 800 | 1900 | 8 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `scryfall_cards` | 831 | 1848 | 5 | 164 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `chembl_molecules` | 973 | 2043 | 10 | 97 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `eia_petroleum_prices` | 988 | 3952 | 2 | 494 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `gleif_lei_records_api` | 1000 | 1800 | 5 | 200 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `arxiv_ai_recent` | 1000 | 2500 | 10 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `gwas_catalog_studies` | 1000 | 2600 | 10 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `pride_projects_search` | 1000 | 3200 | 10 | 100 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `uniprot_human_reviewed_lengths` | 1000 | 4000 | 2 | 500 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `uniprot_reviewed_human` | 1000 | 4000 | 2 | 500 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `worldbank_gdp_constant` | 1140 | 2660 | 6 | 190 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `geoboundaries_all_adm0` | 1150 | 3680 | 5 | 230 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `owid_cement_co2_annual` | 1155 | 4620 | 10 | 97 | 1155 | 10 | `aggregate_floor,median_sample_floor` |
| `iris_seismon_events_fixed` | 1170 | 5330 | 9 | 130 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `worldbank_population_total` | 1176 | 2744 | 6 | 196 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `scryfall_default_cards` | 1225 | 2450 | 7 | 175 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `openalex_authors_large` | 1400 | 3000 | 7 | 200 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `owid_co2_per_gdp_annual` | 1403 | 5612 | 10 | 140 | 1403 | 10 | `aggregate_floor,median_sample_floor` |
| `gbif_occurrence_large` | 1499 | 5999 | 5 | 300 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `plos_search` | 1500 | 3500 | 3 | 500 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `ena_portal_search` | 1500 | 6000 | 3 | 500 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `gbif_datasets` | 1500 | 6000 | 3 | 500 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `internetarchive_advancedsearch` | 1500 | 7000 | 3 | 500 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `crossref_works_large_retry` | 1500 | 8000 | 3 | 500 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `eurostat_retail_trade_volume_monthly` | 1524 | 6096 | 5 | 288 | 1524 | 5 | `aggregate_floor,median_sample_floor` |
| `coinpaprika_exchanges` | 1530 | 5760 | 6 | 255 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `owid_co2_including_luc_annual` | 1545 | 6180 | 10 | 163 | 1545 | 10 | `aggregate_floor,median_sample_floor` |
| `owid_gas_co2_annual` | 1546 | 6184 | 10 | 139.5 | 1546 | 10 | `aggregate_floor,median_sample_floor` |
| `openalex_institutions_large` | 1600 | 4600 | 8 | 200 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `owid_oil_co2_annual` | 1607 | 6428 | 10 | 148.5 | 1607 | 10 | `aggregate_floor,median_sample_floor` |
| `owid_co2_annual` | 1687 | 6748 | 10 | 163 | 1687 | 10 | `aggregate_floor,median_sample_floor` |
| `owid_co2_per_capita_annual` | 1687 | 6748 | 10 | 163 | 1687 | 10 | `aggregate_floor,median_sample_floor` |
| `owid_coal_co2_annual` | 1687 | 6748 | 10 | 163 | 1687 | 10 | `aggregate_floor,median_sample_floor` |
| `tvmaze_shows` | 1715 | 4655 | 7 | 245 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `owid_temperature_change_from_co2_annual` | 1740 | 6960 | 10 | 174 | 1740 | 10 | `aggregate_floor,median_sample_floor` |
| `owid_land_use_change_co2_annual` | 1750 | 7000 | 10 | 175 | 1750 | 10 | `aggregate_floor,median_sample_floor` |
| `owid_methane_annual` | 1750 | 7000 | 10 | 175 | 1750 | 10 | `aggregate_floor,median_sample_floor` |
| `owid_nitrous_oxide_annual` | 1750 | 7000 | 10 | 175 | 1750 | 10 | `aggregate_floor,median_sample_floor` |
| `owid_total_ghg_annual` | 1750 | 7000 | 10 | 175 | 1750 | 10 | `aggregate_floor,median_sample_floor` |
| `openalex_sources_large` | 1800 | 5400 | 9 | 200 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `eurostat_hicp_all_items_monthly` | 1800 | 7200 | 5 | 360 | 1800 | 5 | `aggregate_floor,median_sample_floor` |
| `eurostat_hicp_food_monthly` | 1800 | 7200 | 5 | 360 | 1800 | 5 | `aggregate_floor,median_sample_floor` |
| `eurostat_industrial_production_monthly` | 1884 | 7536 | 5 | 396 | 1884 | 5 | `aggregate_floor,median_sample_floor` |
| `europepmc_grants` | 1984 | 3968 | 4 | 496 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `gdc_cases` | 2000 | 3500 | 4 | 500 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `chembl_documents` | 2000 | 4000 | 4 | 500 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `wikimedia_mostread` | 2000 | 6000 | 2 | 1000 | 0 | 0 | `aggregate_floor` |
| `npi_registry_ca` | 2000 | 7000 | 4 | 500 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `nvd_cpe_match_feed` | 2000 | 7000 | 4 | 500 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `dataone_solr_1000` | 2000 | 10000 | 2 | 1000 | 0 | 0 | `aggregate_floor` |
| `npm_search_packages_large` | 2000 | 10250 | 8 | 250 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `nih_reporter_projects` | 2140 | 9416 | 5 | 428 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `fred_sp500_daily` | 2162 | 8648 | 9 | 251 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `geofabrik_index` | 2220 | 17760 | 4 | 555 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `disease_sh_countries` | 2310 | 14784 | 10 | 231 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `eurostat_female_unemployment_monthly` | 2465 | 9860 | 5 | 520 | 2465 | 5 | `aggregate_floor,median_sample_floor` |
| `eurostat_male_unemployment_monthly` | 2465 | 9860 | 5 | 520 | 2465 | 5 | `aggregate_floor,median_sample_floor` |
| `eurostat_unemployment_monthly` | 2465 | 9860 | 5 | 520 | 2465 | 5 | `aggregate_floor,median_sample_floor` |
| `openfda_device_event` | 2500 | 5500 | 5 | 500 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `openfda_food_enforcement` | 2500 | 7500 | 5 | 500 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `fred_treasury_10y_daily` | 2501 | 10004 | 10 | 250 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `fred_treasury_2y_daily` | 2501 | 10004 | 10 | 250 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `fred_treasury_30y_daily` | 2501 | 10004 | 10 | 250 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `fred_wti_crude_daily` | 2505 | 10020 | 10 | 250.5 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `wikimedia_pageviews_daily` | 2562 | 10248 | 7 | 366 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `rubygems_versions_large` | 2580 | 9288 | 5 | 516 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `chesscom_hikaru_2024_01` | 3135 | 8360 | 3 | 1045 | 0 | 0 | `aggregate_floor` |
| `openlibrary_editions` | 3479 | 6958 | 7 | 497 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `eia_series_petroleum` | 3500 | 5500 | 7 | 500 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `loc_photos_search_large` | 3500 | 8000 | 7 | 500 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `smithsonian_search_large` | 3500 | 10000 | 7 | 500 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `europepmc_grants_large_retry` | 4000 | 8000 | 4 | 1000 | 0 | 0 | `aggregate_floor` |
| `huggingface_models_large` | 4000 | 13000 | 8 | 500 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `taginfo_tags_popular` | 4000 | 19500 | 8 | 500 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `taginfo_keys_all` | 4000 | 25500 | 8 | 500 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `mitre_attack_enterprise` | 4290 | 5148 | 5 | 858 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `usgs_daily_values_large` | 4445 | 35560 | 7 | 572 | 4445 | 7 | `aggregate_floor,median_sample_floor` |
| `inspirehep_literature` | 4927 | 15854 | 5 | 1000 | 0 | 0 | `aggregate_floor` |
| `usgs_water_sites_rdb` | 4992 | 36976 | 6 | 878 | 0 | 0 | `aggregate_floor,median_sample_floor` |
| `crossref_funders_large` | 5000 | 10000 | 5 | 1000 | 0 | 0 | `aggregate_floor` |
| `dataone_solr` | 5039 | 25078 | 6 | 1000 | 0 | 0 | `aggregate_floor` |
| `dataone_solr_large_retry` | 5039 | 25078 | 6 | 1000 | 0 | 0 | `aggregate_floor` |
| `jhu_covid19_confirmed_global_daily` | 5715 | 22860 | 5 | 1143 | 5715 | 5 | `aggregate_floor` |
| `jhu_covid19_confirmed_us_daily` | 5715 | 22860 | 5 | 1143 | 5715 | 5 | `aggregate_floor` |
| `jhu_covid19_deaths_global_daily` | 5715 | 22860 | 5 | 1143 | 5715 | 5 | `aggregate_floor` |
| `jhu_covid19_deaths_us_daily` | 5715 | 22860 | 5 | 1143 | 5715 | 5 | `aggregate_floor` |
| `federalregister_documents_large` | 6000 | 9000 | 6 | 1000 | 0 | 0 | `aggregate_floor` |
| `openml_runs_large` | 6000 | 28000 | 6 | 1000 | 0 | 0 | `aggregate_floor` |
| `figshare_articles_large` | 6000 | 34000 | 6 | 1000 | 0 | 0 | `aggregate_floor` |
| `jpl_cad_2024` | 6552 | 52416 | 4 | 1638 | 0 | 0 | `aggregate_floor` |
| `nasa_donki_flr` | 6768 | 16920 | 6 | 1128 | 0 | 0 | `aggregate_floor` |
| `openml_tasks_large` | 7000 | 26000 | 7 | 1000 | 0 | 0 | `aggregate_floor` |
| `coinpaprika_tickers` | 8000 | 52000 | 4 | 2000 | 0 | 0 | `aggregate_floor` |
| `cisa_kev_catalog` | 8060 | 14508 | 5 | 1612 | 0 | 0 | `aggregate_floor` |
| `nasa_donki_cme` | 9072 | 24192 | 6 | 1512 | 0 | 0 | `aggregate_floor` |

