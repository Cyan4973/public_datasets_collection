...
obj = json.load(open(download_dir / 'federalregister_documents_large.json', encoding='utf-8'))['results']
meta = {
...
for row in obj:
    try:
        dt = datetime.strptime(row['publication_date'], '%Y-%m-%d')
...
for sid, (kind, bits, code) in meta.items():
    ...
    rows.append({'dataset_id': 'federalregister_documents_large', 'series_id': sid, 'sample_path': out.relative_to(data_root).as_posix(), 'numeric_kind': kind, 'bit_width': bits, 'endianness': 'little', 'element_size_bytes': bits // 8, 'sample_size_bytes': out.stat().st_size, 'value_count': len(values)})
(filter_dir / 'ingest_stats.json').write_text(json.dumps({'dataset_id': 'federalregister_documents_large', 'rows_total': len(obj), 'rows_skipped': skipped}, indent=2, sort_keys=True) + '\n', encoding='utf-8')
