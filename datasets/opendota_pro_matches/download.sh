...
if ! curl --globoff -L --fail --retry 2 --retry-delay 2 -A 'openzl-public-datasets/1.0' -o "$TMP" "$URL"; then
  ...
  printf 'opendota_pro_matches\tcurl_failed\n' >> "$FAIL_TSV"
  rm -f "$TMP"
  cp "$LOG_FILE" "$LATEST_LOG" 2>/dev/null || true
  exit 0
fi
...
then
  ...
  printf 'opendota_pro_matches\tvalidation_failed\n' >> "$FAIL_TSV"
  rm -f "$TMP"
  cp "$LOG_FILE" "$LATEST_LOG" 2>/dev/null || true
  exit 0
fi
