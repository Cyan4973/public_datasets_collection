...
PROBE_CACHE_FILE="$REPO_ROOT/$DATA_DIR/downloads/macrostrat_more_numeric_sources/macrostrat_columns_long.json"
...
URL="https://macrostrat.org/api/columns?response=long"
OUT="$DOWNLOAD_DIR/macrostrat_columns.json"
...
if not isinstance(data, list) or len(data) < 3000:
    raise SystemExit("bad Macrostrat columns payload")
required = {"t_age", "b_age", "col_area", "max_thick", "max_min_thick", "min_min_thick", "pbdb_collections", "t_sections", "t_units"}
