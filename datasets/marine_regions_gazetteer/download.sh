...
OUT="$DOWNLOAD_DIR/marine_regions_gazetteer.json"
TMP="$OUT.tmp"
...
rm -f "$TMP"
URL="https://www.marineregions.org/rest/getGazetteerRecordsByName.json/Atlantic%20Ocean/like/false/true/"
if ! curl --globoff -fL --retry 2 --retry-delay 2 -A "openzl-public-datasets/1.0" -o "$TMP" "$URL"; then
  echo -e "marine_regions_gazetteer\tcurl_failed\t$URL" >> "$FAIL_FILE"; rm -f "$TMP"; exit 1
fi
python3 - <<'PY' "$TMP"
import json, sys
