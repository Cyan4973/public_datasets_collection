...
if len(rows) < 10:
    raise SystemExit(f"expected at least 10 shapefile samples, found {len(rows)}")
sizes = []
for row in rows:
    ...
    if len(payload) < 100 or struct.unpack(">I", payload[:4])[0] != 9994 or struct.unpack("<I", payload[28:32])[0] != 1000:
        raise SystemExit(f"invalid shapefile header: {path}")
    ...
    if len(set(payload)) < 2:
        raise SystemExit(f"degenerate shapefile sample: {path}")
    sizes.append(len(payload))
if len(set(sizes)) < 2:
    raise SystemExit("all shapefile samples have identical size")
