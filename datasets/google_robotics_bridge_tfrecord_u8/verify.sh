...
required = {
    "bridge_train_00000_tfrecord_payload_u8",
    "bridge_train_00000_tfrecord_record_lengths_u32",
    "bridge_train_00000_tfrecord_masked_crc_u32",
}
if set(by_id) != required:
    raise SystemExit(f"unexpected series ids: {sorted(by_id)}")

payload_row = by_id["bridge_train_00000_tfrecord_payload_u8"]
length_row = by_id["bridge_train_00000_tfrecord_record_lengths_u32"]
crc_row = by_id["bridge_train_00000_tfrecord_masked_crc_u32"]
for row in rows:
...
if payload_size < MIN_PAYLOAD_BYTES:
    raise SystemExit(f"payload below floor: {payload_size} < {MIN_PAYLOAD_BYTES}")
if int(payload_row["value_count"]) != payload_size or int(payload_row["element_size_bytes"]) != 1:
    raise SystemExit("payload metadata mismatch")
with payload_path.open("rb") as fh:
    first = fh.read(1024 * 1024)
    fh.seek(max(0, payload_size // 2 - 512 * 1024))
    middle = fh.read(1024 * 1024)
    fh.seek(max(0, payload_size - 1024 * 1024))
    last = fh.read(1024 * 1024)
if len(set(first + middle + last)) < 32:
