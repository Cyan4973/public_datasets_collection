...
# Official acceptance floor: >=10000 primary values OR >=102400 primary bytes, plus median >=1000.
...
MIN_ROWS_KEPT = 1_500
EXPECTED_PRIMARY = {
    "donki_flr_begin_epoch_u32",
    "donki_flr_rise_seconds_u32",
    "donki_flr_decay_seconds_u32",
    "donki_flr_active_region_u32",
    "donki_flr_peak_flux_f32",
    "donki_flr_source_lat_f32",
    "donki_flr_source_lon_f32",
}
...
if primary_values < MIN_PRIMARY_VALUES and primary_bytes < MIN_PRIMARY_BYTES:
    raise SystemExit(f"aggregate floor not met: values={primary_values} bytes={primary_bytes}")
