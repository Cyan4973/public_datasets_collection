...
expected = {
    "nomis_obs_value": ("uint", 32, 4, "<I"),
    "nomis_geography": ("uint", 32, 4, "<I"),
    "nomis_sex": ("uint", 8, 1, "<B"),
    "nomis_year": ("uint", 16, 2, "<H"),
    "nomis_month": ("uint", 8, 1, "<B"),
}
...
for row in primary:
    ...
    if row.get("sample_geometry") != "nomis_employment_observation_column" or int(row.get("sample_rank", 0)) != 1:
