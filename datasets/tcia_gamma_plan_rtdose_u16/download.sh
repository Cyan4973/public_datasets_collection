...
curl --fail --silent --show-error --location --retry 5 --retry-delay 2 \
  ...
  "$BASE_URL/getSeries?Collection=Vestibular-Schwannoma-SEG&Modality=RTDOSE"
...
expected = {
    "1.3.6.1.4.1.14519.5.2.1.324778227810006693629648508840410665135": 8200242,
    "1.3.6.1.4.1.14519.5.2.1.9363649087805920279178314503691639909": 8243092,
    "1.3.6.1.4.1.14519.5.2.1.161268545793977743134322234540269044021": 8560602,
}
...
for row in rows:
    ...
    if uid not in expected:
        continue
    if field(row, "Collection") != "Vestibular-Schwannoma-SEG":
        raise SystemExit(f"collection changed for {uid}")
    if field(row, "Modality") != "RTDOSE":
        raise SystemExit(f"modality changed for {uid}")
...
print("metadata_validation=ok series=3 license=CC-BY-4.0")
PY
