...
URL='https://archive.org/advancedsearch.php?q=mediatype:texts&fl[]=downloads&fl[]=item_size&fl[]=week&rows=500&page=1&output=json'
...
if [ -s "$OUT" ] && [ "${FORCE_DOWNLOAD:-0}" != "1" ]; then
  ...
  jq -e '.response.docs != null' "$TMP" >/dev/null
  mv "$TMP" "$OUT"
fi
