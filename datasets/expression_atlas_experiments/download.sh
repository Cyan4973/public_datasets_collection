...
OUT="$DOWNLOAD_DIR/expression_atlas_experiments.json"
TMP="$OUT.tmp"
...
if not (isinstance(obj, dict) and isinstance(obj.get("experiments"), list)):
    raise SystemExit("bad expression_atlas_experiments payload")
PY
  mv "$TMP" "$OUT"
fi
