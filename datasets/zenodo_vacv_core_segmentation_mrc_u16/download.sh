#!/usr/bin/env bash
# Download and validate the exact CC BY 4.0 VACV segmentation volume.
set -euo pipefail
...
RECORD_JSON="$DOWNLOAD_DIR/zenodo_record_20262954.json"
curl --fail --silent --show-error --location --retry 5 --max-time 120 \
  --output "$RECORD_JSON.part" "https://zenodo.org/api/records/20262954"
...
if int(obj.get("id", 0)) != 20262954:
    raise SystemExit("unexpected Zenodo record id")
metadata = obj.get("metadata", {})
if metadata.get("title") != "3D segmentation for VACV cores":
...
NAME="032_original_ground_truth.mrc"
SIZE="107649024"
MD5="a06b214d041398b9d0f0e8702dce8d7a"
URL="https://zenodo.org/api/records/20262954/files/032_original_ground_truth.mrc/content"
