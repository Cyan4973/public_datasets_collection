...
if len(candidates) < 50:
    raise SystemExit(f"only {len(candidates)} candidate sites discovered")
PY
...
while IFS='	' read -r rank state_code site_no station_nm instruments_cd; do
  ...
  for year in $(seq "${START_YEAR}" "${END_YEAR}"); do
    ...
    if fetch_daily "${url}" "${out}"; then
      ...
      say "ok daily ${site_no} ${year} ${out}"
    else
      status=$?
      if [ "${status}" -eq 2 ]; then
        daily_cached=$((daily_cached + 1))
        say "cached daily ${site_no} ${year} ${out}"
      else
    ...
    if daily_payload_has_usable_values "${out}"; then
      site_has_data=1
      if [ -z "${usable_years}" ]; then
        usable_years="${year}"
      else
        usable_years="${usable_years},${year}"
      fi
    fi
  done
...
if [ "${daily_failures}" -gt 0 ]; then
  say "daily fetch completed with failures"
  exit 1
fi
