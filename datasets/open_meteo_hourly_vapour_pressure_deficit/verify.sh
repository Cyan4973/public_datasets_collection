...
DOWNLOAD_ROOT="${DATA_DIR}/downloads/open_meteo_hourly_vapour_pressure_deficit"
FILTERED_ROOT="${DATA_DIR}/filtered/open_meteo_hourly_vapour_pressure_deficit"
INDEX_ROOT="${DATA_DIR}/index/open_meteo_hourly_vapour_pressure_deficit"
SAMPLES_ROOT="${DATA_DIR}/samples/open_meteo_hourly_vapour_pressure_deficit"
LOG_ROOT="${DATA_DIR}/logs/open_meteo_hourly_vapour_pressure_deficit"
...
dataset_id = "open_meteo_hourly_vapour_pressure_deficit"
parameter_id = "vapour_pressure_deficit"
...
stats_by_key = {(row["location_id"], row["year"]): row for row in stats_rows}
    ...
    for year in range(2022, 2024):
        ...
        for ts, raw_value in zip(times, values):
            ...
            try:
                ...
                _, month_s, day_s = date_part.split("-")
            ...
            if obs_month < 1 or obs_month > 12 or obs_day < 1 or obs_day > 31 or obs_hour < 0 or obs_hour > 23:
                ...
                continue
            kept_count += 1
            group_counts[location_id] += 1
            if start_time == "":
                start_time = str(ts)
            end_time = str(ts)
        stats_row = stats_by_key.get((location_id, str(year)))
        if stats_row is None:
            raise SystemExit(f"missing stats row for {location_id} {year}")
        for field, value in [("row_count", row_count), ("kept_count", kept_count), ("skipped_blank_count", skipped_blank_count), ("skipped_parse_count", skipped_parse_count)]:
            if int(stats_row[field]) != value:
                raise SystemExit(f"stats mismatch for {location_id} {year} field {field}: stats={stats_row[field]} raw={value}")
        if stats_row["start_time"] != start_time:
            raise SystemExit(f"start time mismatch for {location_id} {year}")
        if stats_row["end_time"] != end_time:
            raise SystemExit(f"end time mismatch for {location_id} {year}")

for location_id, value_count in sorted(group_counts.items()):
