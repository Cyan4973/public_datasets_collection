...
- `download.sh` validates that each JSON payload contains `hourly.time` and
  `hourly.dew_point_2m` with matching lengths before accepting it into cache.
...
```sh
bash datasets/open_meteo_hourly_dew_point/download.sh
bash datasets/open_meteo_hourly_dew_point/build.sh
bash datasets/open_meteo_hourly_dew_point/verify.sh
```
