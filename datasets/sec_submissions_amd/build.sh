...
recent=json.load(open(download_dir/'amd.json', encoding='utf-8'))['filings']['recent']
series_defs=[
...
with (filter_dir/'company_stats.tsv').open('w', encoding='utf-8', newline='') as f:
 w=csv.writer(f, delimiter='\t'); w.writerow(['company_id','row_count','kept_count','skipped_count']); w.writerow(['amd',row_count,kept,skipped])
for s in series_defs:
 arr=array.array(s['array_type'], vals[s['series_id']])
 if arr.itemsize > 1 and os.sys.byteorder != 'little': arr.byteswap()
 out=samples_dir/s['series_id']/f'amd.bin'
 with out.open('wb') as fh: fh.write(arr.tobytes())
 records.append({'dataset_id':'sec_submissions_amd','series_id':s['series_id'],'sample_path':out.relative_to(data_root).as_posix(),'numeric_kind':s['numeric_kind'],'bit_width':s['bit_width'],'endianness':s['endianness'],'element_size_bytes':s['element_size_bytes'],'sample_size_bytes':out.stat().st_size,'value_count':len(vals[s['series_id']]),'company_id':'amd','cik':'0000002488'})
