...
HEADER_BYTES="${WORLDCOVER_HEADER_BYTES:-2097152}"
TILES_PER_SOURCE="${WORLDCOVER_TILES_PER_SOURCE:-8}"
UA="openzl-public-datasets/1.0 (numeric dataset collection)"
SOURCE_URLS="$DOWNLOAD_DIR/source_urls.tsv"
PLAN="$DOWNLOAD_DIR/download_plan.tsv"

if [ -n "${WORLDCOVER_URLS_FILE:-}" ]; then
  cp "$WORLDCOVER_URLS_FILE" "$SOURCE_URLS"
else
  cat > "$SOURCE_URLS" <<'EOF'
source_id	url
N00E006	https://esa-worldcover.s3.eu-central-1.amazonaws.com/v200/2021/map/ESA_WorldCover_10m_2021_v200_N00E006_Map.tif
N39W123	https://esa-worldcover.s3.eu-central-1.amazonaws.com/v200/2021/map/ESA_WorldCover_10m_2021_v200_N39W123_Map.tif
N51E000	https://esa-worldcover.s3.eu-central-1.amazonaws.com/v200/2021/map/ESA_WorldCover_10m_2021_v200_N51E000_Map.tif
S03E036	https://esa-worldcover.s3.eu-central-1.amazonaws.com/v200/2021/map/ESA_WorldCover_10m_2021_v200_S03E036_Map.tif
S24E132	https://esa-worldcover.s3.eu-central-1.amazonaws.com/v200/2021/map/ESA_WorldCover_10m_2021_v200_S24E132_Map.tif
N27E075	https://esa-worldcover.s3.eu-central-1.amazonaws.com/v200/2021/map/ESA_WorldCover_10m_2021_v200_N27E075_Map.tif
S12W072	https://esa-worldcover.s3.eu-central-1.amazonaws.com/v200/2021/map/ESA_WorldCover_10m_2021_v200_S12W072_Map.tif
N60E024	https://esa-worldcover.s3.eu-central-1.amazonaws.com/v200/2021/map/ESA_WorldCover_10m_2021_v200_N60E024_Map.tif
EOF
fi

if [ "$HEADER_BYTES" -lt 65536 ]; then
  echo "FATAL: WORLDCOVER_HEADER_BYTES must be at least 65536" >&2
  exit 1
fi
...
tail -n +2 "$SOURCE_URLS" | while IFS=$'\t' read -r source_id url; do
  ...
  if [ -s "$out" ] && [ "${FORCE_DOWNLOAD:-0}" != "1" ]; then
    echo "header cache_hit source=$source_id path=$out"
  else
...
export SOURCE_URLS HEADER_DIR PLAN HEADER_BYTES TILES_PER_SOURCE
python3 - <<'PY'
...
TYPE_SIZES = {1: 1, 2: 1, 3: 2, 4: 4, 5: 8, 7: 1, 8: 2, 9: 4, 10: 8, 11: 4, 12: 8, 16: 8, 17: 8, 18: 8}
...
def parse_tiff_header(path: Path) -> dict[str, object]:
    ...
    if bits != 8 or samples_per_pixel != 1 or sample_format != 1:
        raise ValueError(f"{path.name}: not single-band uint8 bits={bits} samples={samples_per_pixel} sample_format={sample_format}")
...
with plan_path.open("w", encoding="utf-8", newline="") as out:
    ...
    for source in sources:
        ...
        for tile_index, tile_x, tile_y in select_indices(int(info["tiles_across"]), int(info["tiles_down"]), tiles_per_source):
...
with plan.open("r", encoding="utf-8", newline="") as fh:
    for row in csv.DictReader(fh, delimiter="\t"):
        rows += 1
        p = download_dir / row["chunk_path"]
        expected = int(row["byte_count"])
        if not p.is_file():
            raise SystemExit(f"missing chunk {p}")
        actual = p.stat().st_size
        if actual != expected:
            raise SystemExit(f"chunk size mismatch {p}: {actual} != {expected}")
        total += actual
if rows < 8:
