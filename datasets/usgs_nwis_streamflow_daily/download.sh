...
PARAMETER_CD="00060"
PARAMETER_LABEL="streamflow"
START_DATE=${USGS_NWIS_STREAMFLOW_START_DATE:-"2000-01-01"}
END_DATE=${USGS_NWIS_STREAMFLOW_END_DATE:-"2024-12-31"}
TARGET_SITES=${USGS_NWIS_STREAMFLOW_TARGET_SITES:-40}
TARGET_CANDIDATES=${USGS_NWIS_STREAMFLOW_TARGET_CANDIDATES:-500}
    ...
    for wrapper in selected_series.get("values", []):
        ...
        for row in rows:
            ...
            if obs_year < 0 or obs_year > 65535 or obs_month < 1 or obs_month > 12 or obs_day < 1 or obs_day > 31:
                continue
            wrapper_count += 1
        if wrapper_count > best_wrapper_count:
            best_wrapper_count = wrapper_count
            best_wrapper_rows = rows

    for row in best_wrapper_rows:
        raw_value = str(row.get("value", "")).strip()
        raw_date = str(row.get("dateTime", "")).strip()
        if raw_value == "" or raw_date == "":
            continue
