...
- `download.sh` validates that each JSON payload contains `hourly.time` and
  `hourly.temperature_2m` with matching lengths before accepting it into cache.
...
```sh
bash datasets/open_meteo_hourly_temperature/download.sh
bash datasets/open_meteo_hourly_temperature/build.sh
bash datasets/open_meteo_hourly_temperature/verify.sh
```
