...
ALLOWED = {"fedreg_page_length_u16", "fedreg_start_page_u32"}
CODE = {16: "H", 32: "I"}
...
def unpack(path: Path, count: int, bits: int) -> tuple:
    data = path.read_bytes()
    esz = bits // 8
    if len(data) != count * esz:
        raise SystemExit(f"size mismatch for {path}")
    return struct.unpack("<" + CODE[bits] * count, data)


for r in primary:
    p = root / r["sample_path"]
    bits = int(r["bit_width"])
    if not p.is_file():
    ...
    if len(set(unpack(p, int(r["value_count"]), bits))) <= 1:
