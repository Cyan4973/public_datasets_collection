...
allowed = {
    "ooni_measurement_unix_u32": ("primary", "uint", 32, 4, "I"),
    "ooni_probe_asn_u32": ("primary", "uint", 32, 4, "I"),
    "ooni_blocking_general_score_f32": ("primary", "float", 32, 4, "f"),
}
...
if int(stats["retained_measurements"]) != primary_counts[0]:
    raise SystemExit("retained measurement count does not match sample count")

print(
    f"verified_samples={len(rows)} retained_measurements={stats['retained_measurements']} "
