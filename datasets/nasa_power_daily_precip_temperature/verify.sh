...
MIN_VALUES_PER_SAMPLE=${NASA_POWER_DAILY_PRECIP_TEMPERATURE_MIN_VALUES_PER_SAMPLE:-15000}
DOWNLOAD_ROOT="${DATA_DIR}/downloads/nasa_power_daily_precip_temperature"
FILTERED_ROOT="${DATA_DIR}/filtered/nasa_power_daily_precip_temperature"
INDEX_ROOT="${DATA_DIR}/index/nasa_power_daily_precip_temperature"
SAMPLES_ROOT="${DATA_DIR}/samples/nasa_power_daily_precip_temperature"
LOG_ROOT="${DATA_DIR}/logs/nasa_power_daily_precip_temperature"
...
parameter_id = "PRECTOTCORR"
series = {"series_id": "nasa_power_precip_prectotcorr_f64", "numeric_kind": "float", "bit_width": 64, "endianness": "little", "element_size_bytes": 8}
...
expected_key_count = len(locations)
if len(expected_records) != expected_key_count:
    raise SystemExit(f"expected {expected_key_count} location samples, got {len(expected_records)}")
