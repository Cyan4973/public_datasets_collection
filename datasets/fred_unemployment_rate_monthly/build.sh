...
DOWNLOAD_ROOT="${DATA_DIR}/downloads/fred_unemployment_rate_monthly"
FILTERED_ROOT="${DATA_DIR}/filtered/fred_unemployment_rate_monthly"
INDEX_ROOT="${DATA_DIR}/index/fred_unemployment_rate_monthly"
SAMPLES_ROOT="${DATA_DIR}/samples/fred_unemployment_rate_monthly"
LOG_ROOT="${DATA_DIR}/logs/fred_unemployment_rate_monthly"
...
dataset_id = "fred_unemployment_rate_monthly"
...
path = download_root / "fred_unemployment_rate_monthly.csv"
if not path.is_file():
...
for year in sorted(year_values):
    payloads = {
        "unemployment_rate_f32": year_values[year],
    }
    for series in series_defs:
