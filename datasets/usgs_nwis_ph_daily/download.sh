...
PARAMETER_CD="00400"
STAT_CD="00003"
START_DATE="${USGS_NWIS_PH_START_DATE:-1980-01-01}"
END_DATE="${USGS_NWIS_PH_END_DATE:-2024-12-31}"
STATES="${USGS_NWIS_PH_STATES:-al ak az ar ca co ct de fl ga id il in ia ks ky la me md ma mi mn ms mo mt ne nv nh nj nm ny nc nd oh ok or pa ri sc sd tn tx ut vt va wa wv wi wy}"
MIN_VALUES_PER_SAMPLE="${USGS_NWIS_PH_MIN_VALUES_PER_SAMPLE:-3000}"
MIN_SAMPLE_COUNT="${USGS_NWIS_PH_MIN_SAMPLE_COUNT:-20}"
REQUEST_DELAY="${USGS_NWIS_PH_REQUEST_DELAY_SECONDS:-1.0}"
...
def parse_row(row: object) -> tuple[float, str] | None:
    ...
    try:
        ...
        return None
    if not math.isfinite(value) or value < 0 or value > 14:
        return None
