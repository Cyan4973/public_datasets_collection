...
with plan_path.open("w", encoding="utf-8", newline="") as plan_file:
    for site in sites:
        for year in range(2021, 2024):
            ...
            out = f"dv_{site}_{year}.json"
            plan_file.write(f"{site}\t{year}\t{url}\t{out}\n")
PY
...
while IFS='	' read -r site year url rel_out; do
  ...
  say "fetch ${site} ${year} ${url}"
  if fetch "${url}" "${out}"; then
    success_count=$((success_count + 1))
    say "ok ${site} ${year} ${out}"
  else
    status=$?
    if [ "${status}" -eq 2 ]; then
      cached_count=$((cached_count + 1))
      say "cached ${site} ${year} ${out}"
    else
      failure_count=$((failure_count + 1))
      rm -f "${out}" "${out}.tmp"
      printf '%s\t%s\t%s\t%s\n' "${site}" "${year}" "${url}" "${rel_out}" >> "${FAILURES_FILE}"
      say "failed ${site} ${year} ${url}"
    fi
  fi
done < "${PLAN_FILE}"

find "${DOWNLOAD_ROOT}" -maxdepth 1 -type f -name 'dv_*.json' -print0 \
  | sort -z \
