...
ANCHOR_FAMILY = "usgs_streamflow_cfs_f64"
...
if len(by_family) < MIN_FAMILIES:
    raise SystemExit(f"only {len(by_family)} families (< {MIN_FAMILIES}): {sorted(by_family)}")
if ANCHOR_FAMILY not in by_family:
    raise SystemExit(f"anchor family {ANCHOR_FAMILY} missing; got {sorted(by_family)}")
...
for fam, samples in by_family.items():
    ...
    if fam_bytes > MAX_FAMILY_BYTES:
        raise SystemExit(f"family {fam} bytes exceed cap: {fam_bytes}")
    # one family = one numeric kind/width
...
for r in primary:
    ...
    values = unpack(p, int(r["value_count"]))
    if len(set(values)) <= 1:
