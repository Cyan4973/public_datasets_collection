...
params = {"id": "CPILFESL", "cosd": "2015-01-01", "coed": "2024-12-31"}
url = base_url + "?" + urllib.parse.urlencode(params)
with plan_path.open("w", encoding="utf-8", newline="") as plan_file:
    plan_file.write(f"CPILFESL\t{url}\tfred_core_cpi_monthly.csv\n")
PY
...
with path.open("r", encoding="utf-8", newline="") as handle:
    ...
    if reader.fieldnames != ["observation_date", "CPILFESL"]:
