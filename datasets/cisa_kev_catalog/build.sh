...
for row in items:
    try:
        vals['cisa_kev_cwe_count'].append(len(row.get('cwes') or []))
        vals['cisa_kev_known_ransomware'].append(1 if str(row.get('knownRansomwareCampaignUse','')).strip().lower() == 'known' else 0)
