...
expected = {
    "gbif_record_count_u64": ("uint", 64, 8, "<Q"),
    "gbif_keyword_count_u16": ("uint", 16, 2, "<H"),
    "gbif_decade_count_u16": ("uint", 16, 2, "<H"),
}
...
for row in primary:
    ...
    if row.get("sample_geometry") != "gbif_dataset_table_column" or int(row.get("sample_rank", 0)) != 1:
