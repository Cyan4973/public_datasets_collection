...
if len(rows) < 40:
    raise SystemExit(f"expected at least 40 font samples, found {len(rows)}")
sizes = []
for row in rows:
    ...
    if payload[:4] not in VALID_HEADERS:
        raise SystemExit(f"invalid font header: {path}")
    ...
    if len(set(payload)) < 2:
        raise SystemExit(f"degenerate font sample: {path}")
    sizes.append(len(payload))
if len(set(sizes)) < 2:
    raise SystemExit("all font samples have identical size")
