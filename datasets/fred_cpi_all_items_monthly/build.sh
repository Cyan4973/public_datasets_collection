...
DOWNLOAD_ROOT="${DATA_DIR}/downloads/fred_cpi_all_items_monthly"
FILTERED_ROOT="${DATA_DIR}/filtered/fred_cpi_all_items_monthly"
INDEX_ROOT="${DATA_DIR}/index/fred_cpi_all_items_monthly"
SAMPLES_ROOT="${DATA_DIR}/samples/fred_cpi_all_items_monthly"
LOG_ROOT="${DATA_DIR}/logs/fred_cpi_all_items_monthly"
...
dataset_id = "fred_cpi_all_items_monthly"
fred_series_id = "CPIAUCSL"
value_column = "CPIAUCSL"
series_defs = [
    {"series_id": "cpi_value_f32", "array_type": "f", "numeric_kind": "float", "bit_width": 32, "endianness": "little", "element_size_bytes": 4},
]
...
path = download_root / "fred_cpi_all_items_monthly.csv"
if not path.is_file():
...
for year in sorted(year_values):
    payloads = {
        "cpi_value_f32": year_values[year],
    }
    for series in series_defs:
