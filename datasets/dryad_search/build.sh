...
for row in items:
    try:
        m=row.get("metrics",{})
        vals["dryad_dataset_id"].append(int(row["id"]))
        vals["dryad_storage_size"].append(int(row["storageSize"]))
...
for sid,(kind,bits,code) in meta.items():
    ...
    rows.append({"dataset_id":"dryad_search","series_id":sid,"sample_path":out.relative_to(data_root).as_posix(),"numeric_kind":kind,"bit_width":bits,"endianness":"little","element_size_bytes":bits//8,"sample_size_bytes":out.stat().st_size,"value_count":len(values)})
(filter_dir/'ingest_stats.json').write_text(json.dumps({"dataset_id":"dryad_search","rows_total":len(items),"rows_skipped":skipped},indent=2,sort_keys=True)+'\n',encoding='utf-8')
