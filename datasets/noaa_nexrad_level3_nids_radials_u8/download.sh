...
PRODUCT_DIR="$DOWNLOAD_DIR/products"
LISTINGS_DIR="$DOWNLOAD_DIR/listings"
mkdir -p "$LOG_DIR" "$PRODUCT_DIR" "$LISTINGS_DIR"
...
UA="${NEXRAD_L3_UA:-openzl-public-datasets/1.0}"
...
if [ -n "${NEXRAD_L3_URLS_FILE:-}" ]; then
  ...
  if [ -n "$PREFIX" ]; then
    ...
    PREFIXES=(
      "${STATION}_${PRODUCT_CODE}_${YEAR}_${MONTH}_${DAY}_"
...
if urls_path:
    with Path(urls_path).open("r", encoding="utf-8", newline="") as fh:
        for row in csv.DictReader(fh, delimiter="\t"):
            ...
            if name and url:
                rows.append({"name": name, "key": name, "url": url, "product_code": row_product})
elif keys_path:
    keys = [
        line.strip()
...
filtered = sorted(
    {(item["name"], item["url"]): item for item in filtered}.values(),
    key=lambda item: (item["name"], item["url"]),
)[:limit]
if not filtered:
    attempted = []
...
with plan.open("r", encoding="utf-8", newline="") as fh:
    for row in csv.DictReader(fh, delimiter="\t"):
        ...
        marker = (row["product_code"] + os.environ["STATION"]).encode("ascii") + b"\r\r\n"
        if marker not in data:
            raise SystemExit(f"{row['name']}: missing AWIPS product marker {marker!r}")
        if b"BZh" not in data:
            raise SystemExit(f"{row['name']}: missing bzip2 NIDS symbology stream")
        records.append({
            "name": row["name"],
