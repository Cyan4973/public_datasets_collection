...
OUT="$DOWNLOAD_DIR/scryfall_cards.json"
TMP="$OUT.tmp"
URL="https://api.scryfall.com/cards/search?q=game%3Apaper&order=set&page=1"
...
if not (isinstance(obj, dict) and isinstance(obj.get('data'), list)):
    raise SystemExit('bad scryfall_cards payload')
PY
  mv "$TMP" "$OUT"
fi
