...
def close_record() -> None:
    ...
    row = {
        ...
        "natural_record_kind": "refseq_viral_fasta_record",
    }
    rows.append(row)
    records.append(
        {
...
if len(rows) < MIN_RECORDS:
    raise RuntimeError(f"too few viral records: {len(rows)} < {MIN_RECORDS}")
stats = {
    "dataset_id": DATASET_ID,
    "source": rel(archive),
    "records": records,
