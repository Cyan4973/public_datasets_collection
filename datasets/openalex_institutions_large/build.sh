...
obj = json.load(open(download_dir / 'openalex_institutions_large.json', encoding='utf-8'))['results']
meta = {
...
for row in obj:
    try:
        ...
        vals['openalex_inst_works_count'].append(int(row['works_count']))
        vals['openalex_inst_cited_by_count'].append(int(row['cited_by_count']))
...
for sid, (kind, bits, code) in meta.items():
    ...
    rows.append({'dataset_id': 'openalex_institutions_large', 'series_id': sid, 'sample_path': out.relative_to(data_root).as_posix(), 'numeric_kind': kind, 'bit_width': bits, 'endianness': 'little', 'element_size_bytes': bits // 8, 'sample_size_bytes': out.stat().st_size, 'value_count': len(values)})
(filter_dir / 'ingest_stats.json').write_text(json.dumps({'dataset_id': 'openalex_institutions_large', 'rows_total': len(obj), 'rows_skipped': skipped}, indent=2, sort_keys=True) + '\n', encoding='utf-8')
