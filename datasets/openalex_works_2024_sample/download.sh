...
DATE_START="${OPENALEX_DATE_START:-2024-01-01}"
DATE_END="${OPENALEX_DATE_END:-2024-12-31}"
...
echo "[$(date -Is)] download_start dataset=$DATASET_ID date_start=$DATE_START date_end=$DATE_END target_records=$TARGET_RECORDS page_size=$PAGE_SIZE"

cursor="*"
rows_downloaded=0
page=0
while [ "$rows_downloaded" -lt "$TARGET_RECORDS" ]; do
  out="$PAGE_DIR/works_2024_page_$(printf '%04d' "$page").json"
  tmp="$out.tmp"
  if [ -s "$out" ] && [ "${FORCE_DOWNLOAD:-0}" != "1" ]; then
    ...
    CURL_ARGS=(
      ...
      -o "$tmp"
      --data-urlencode "filter=from_publication_date:$DATE_START,to_publication_date:$DATE_END"
...
python3 - <<'PY' "$PAGE_DIR" "$DOWNLOAD_DIR/download_stats.json" "$TARGET_RECORDS" "$DATE_START" "$DATE_END"
import json
import re
import sys
...
rows_total = 0
api_total = None
for path in sorted(page_dir.glob("works_2024_page_*.json")):
...
if duplicate_ids:
    raise SystemExit(f"duplicate OpenAlex work IDs across pages: {duplicate_ids}")
