...
items=json.load(open(download_dir/"scryfall_cards.json", encoding='utf-8'))["data"]
meta={
    "scryfall_cmc": ("float", 32, "f"),
...
for row in items:
    try:
        vals["scryfall_cmc"].append(float(row["cmc"]))
...
for sid,(kind,bits,code) in meta.items():
    ...
    rows.append({"dataset_id":"scryfall_cards","series_id":sid,"sample_path":out.relative_to(data_root).as_posix(),"numeric_kind":kind,"bit_width":bits,"endianness":"little","element_size_bytes":bits//8,"sample_size_bytes":out.stat().st_size,"value_count":len(values)})
(filter_dir/'ingest_stats.json').write_text(json.dumps({"dataset_id":"scryfall_cards","rows_total":len(items),"rows_skipped":skipped}, indent=2, sort_keys=True) + '\n', encoding='utf-8')
