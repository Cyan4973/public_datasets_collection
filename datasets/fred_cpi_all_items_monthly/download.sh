...
params = {"id": "CPIAUCSL", "cosd": "2015-01-01", "coed": "2024-12-31"}
url = base_url + "?" + urllib.parse.urlencode(params)
with plan_path.open("w", encoding="utf-8", newline="") as plan_file:
    plan_file.write(f"CPIAUCSL\t{url}\tfred_cpi_all_items_monthly.csv\n")
PY
...
with path.open("r", encoding="utf-8", newline="") as handle:
    ...
    if reader.fieldnames != ["observation_date", "CPIAUCSL"]:
