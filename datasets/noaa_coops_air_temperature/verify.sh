...
for station in stations:
    ...
    checks = [("row_count", value_count), ("raw_file_count", len(json_files)), ("raw_row_count", raw_row_count), ("duplicate_rows", duplicate_rows), ("skipped_rows", skipped_rows)]
    for field, raw in checks:
        if int(stats_row[field]) != raw:
            raise SystemExit(f"{field} mismatch for {station['station_slug']}: stats={stats_row[field]} raw={raw}")
    expected_records[(series_id, station["station_slug"])] = {
        "dataset_id": dataset_id, "series_id": series_id, "sample_path": sample_path.relative_to(data_root).as_posix(),
        "numeric_kind": "float", "bit_width": 64, "endianness": "little", "element_size_bytes": 8,
        "sample_size_bytes": sample_size_bytes, "value_count": value_count,
    }
