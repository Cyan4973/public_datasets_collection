...
These are not “bad sources.” They are bad recipe shapes. The current one-recipe-per-indicator split produces dozens of tiny accepted recipes that should become larger bundled family recipes instead.
...
Action:
- replace with bundled FRED family recipes rather than keep 24 thin standalones
...
Action:
- replace with a bundled ECB FX matrix recipe instead of keeping one currency-pair recipe per dataset
...
Action:
- replace with broader multi-indicator World Bank bundles instead of per-indicator recipes
...
Action:
- replace with broader IMF macro bundles instead of per-indicator recipes
...
Action:
- replace with broader OWID bundles instead of per-indicator recipes
...
Action:
- replace with broader Eurostat bundles instead of per-indicator recipes
...
Action:
- replace with a broader multi-metric companyfacts family recipe
...
2. stop carrying fragmented family standalones by planning family-level replacements
