...
DOWNLOAD_ROOT="${DATA_DIR}/downloads/world_bank_urban_population_percent_annual"
FILTERED_ROOT="${DATA_DIR}/filtered/world_bank_urban_population_percent_annual"
INDEX_ROOT="${DATA_DIR}/index/world_bank_urban_population_percent_annual"
SAMPLES_ROOT="${DATA_DIR}/samples/world_bank_urban_population_percent_annual"
LOG_ROOT="${DATA_DIR}/logs/world_bank_urban_population_percent_annual"
...
dataset_id = "world_bank_urban_population_percent_annual"
indicator_id = "SP.URB.TOTL.IN.ZS"
countries = ["USA", "CHN", "IND", "BRA", "DEU", "JPN", "NGA", "MEX", "FRA", "ZAF"]
series_defs = [
    {"series_id": "urban_population_percent_f64", "array_type": "d", "numeric_kind": "float", "bit_width": 64, "endianness": "little", "element_size_bytes": 8},
    ...
    for country_code in countries:
        ...
        for year, value in parsed:
            ...
            if start_year == "":
                start_year = str(year)
            end_year = str(year)
        payloads = {"urban_population_percent_f64": values, "obs_year_u16": years}
        for series in series_defs:
