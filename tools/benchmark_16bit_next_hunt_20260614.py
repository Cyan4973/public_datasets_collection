...
DATASET_IDS = [
    "dwd_radolan_rw_precip_i16",
    "nsynth_test_notes_i16",
]
MIN_VALUES = 10_000
MIN_BYTES = 100 * 1024
...
def describe_geometry(rows: list[dict]) -> str:
    if not rows:
        return "unknown"

    standard_geometries = {str(row["sample_geometry"]) for row in rows if "sample_geometry" in row}
    standard_shapes = {tuple(row["sample_shape"]) for row in rows if "sample_shape" in row}
    if standard_geometries == {"2d_raster"} and len(standard_shapes) == 1:
        height, width = next(iter(standard_shapes))
        return f"2D raster {height}x{width}"
    if standard_geometries == {"1d_waveform"} and len(standard_shapes) == 1:
        (frames,) = next(iter(standard_shapes))
        frame_rates = {int(row["framerate"]) for row in rows if "framerate" in row and str(row["framerate"]) != ""}
        if len(frame_rates) == 1:
            return f"1D waveform {frames} frames @ {next(iter(frame_rates))} Hz"
        return f"1D waveform {frames} frames"
    if standard_geometries:
        return ",".join(sorted(standard_geometries))

    grid_shapes = {
        (int(row["grid_height"]), int(row["grid_width"]))
        for row in rows
        if "grid_height" in row and "grid_width" in row
    }
    if len(grid_shapes) == 1:
        height, width = next(iter(grid_shapes))
        return f"2D raster {height}x{width}"
    if grid_shapes:
        return f"2D raster mixed:{len(grid_shapes)} shapes"

    frame_rates = {int(row["framerate"]) for row in rows if "framerate" in row and str(row["framerate"]) != ""}
    value_counts = {int(row["value_count"]) for row in rows}
    if len(frame_rates) == 1 and len(value_counts) == 1:
        return f"1D waveform {next(iter(value_counts))} frames @ {next(iter(frame_rates))} Hz"
    if frame_rates:
        return f"1D waveform mixed:{len(value_counts)} lengths"
    if len(value_counts) == 1:
        return f"1D array {next(iter(value_counts))} values"
    return f"1D array mixed:{len(value_counts)} lengths"
...
def summarize_dataset(dataset_id: str, data_dir: str) -> dict:
    ...
    for series_id, series_rows in sorted(by_series.items()):
        ...
        if role == "primary":
            ...
            primary_geometries.append(describe_geometry(series_rows))
        series_summaries.append(
            {
                "series_id": series_id,
                "role": role,
                "sample_geometry": describe_geometry(series_rows),
    ...
    sample_geometry = ", ".join(sorted(set(primary_geometries))) if primary_geometries else "unknown"
    base.update(
        {
...
def write_reports(summaries: list[dict], report_prefix: Path) -> None:
    ...
    for summary in summaries:
        ...
        lines.append(
            f"| `{summary['dataset_id']}` | `{summary['status']}` | {summary['primary_samples']} | {summary['primary_bytes']} | {summary['sample_geometry']} | {dist} | "
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build, verify, and summarize next staged 16-bit dataset candidates.")
    ...
    parser.add_argument("--report-prefix", default="reports/16bit_next_hunt_20260614_dataset_state", help="Report path prefix without extension.")
    ...
    for summary in summaries:
        print(
            ...
            f"geometry={summary['sample_geometry']} reasons={','.join(summary['reasons']) or 'none'}"
        )
    return 0
