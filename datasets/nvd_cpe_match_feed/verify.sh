...
expected = {
    "nvd_cpe_last_modified_at_u32": ("uint", 32, 4, "<I"),
    "nvd_cpe_cpe_last_modified_at_u32": ("uint", 32, 4, "<I"),
    "nvd_cpe_match_count_u16": ("uint", 16, 2, "<H"),
}
...
for row in primary:
    ...
    if row.get("sample_geometry") != "nvd_cpe_match_string_column" or int(row.get("sample_rank", 0)) != 1:
