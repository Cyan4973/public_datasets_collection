...
DOWNLOAD_ROOT="${DATA_DIR}/downloads/fred_pce_price_index_monthly"
FILTERED_ROOT="${DATA_DIR}/filtered/fred_pce_price_index_monthly"
INDEX_ROOT="${DATA_DIR}/index/fred_pce_price_index_monthly"
SAMPLES_ROOT="${DATA_DIR}/samples/fred_pce_price_index_monthly"
LOG_ROOT="${DATA_DIR}/logs/fred_pce_price_index_monthly"
...
dataset_id = "fred_pce_price_index_monthly"
fred_series_id = "PCEPI"
value_column = "PCEPI"
series_defs = [
    {"series_id": "pce_price_index_f32", "array_type": "f", "numeric_kind": "float", "bit_width": 32, "endianness": "little", "element_size_bytes": 4},
]
...
path = download_root / "fred_pce_price_index_monthly.csv"
if not path.is_file():
...
for year in sorted(year_values):
    payloads = {
        "pce_price_index_f32": year_values[year],
    }
    for series in series_defs:
