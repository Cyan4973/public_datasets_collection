...
if not records:
    raise SystemExit("sample index is empty — no sites produced usable data")
...
for record in records:
    ...
    expected_size = record.get("sample_size_bytes")
    if actual_size != expected_size:
        raise SystemExit(f"size mismatch for {sample_path}: expected {expected_size}, got {actual_size}")
    value_count = record.get("value_count", 0)
    element_size = record.get("element_size_bytes", 1)
...
sites_verified = len({r["sample_path"].split("/")[-1] for r in records if r.get("series_id") == "usgs_dissolved_oxygen_f64"})
