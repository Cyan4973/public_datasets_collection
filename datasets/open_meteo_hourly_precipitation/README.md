...
precipitation observations and converts them into raw numeric samples.
...
- `download.sh` validates that each JSON payload contains `hourly.time` and
  `hourly.precipitation` with matching lengths before accepting it into cache.
...
```sh
bash datasets/open_meteo_hourly_precipitation/download.sh
bash datasets/open_meteo_hourly_precipitation/build.sh
bash datasets/open_meteo_hourly_precipitation/verify.sh
```
