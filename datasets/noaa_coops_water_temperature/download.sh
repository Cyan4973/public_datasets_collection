...
DATASET_ID="noaa_coops_water_temperature"
...
with plan_path.open("w", encoding="utf-8", newline="") as plan_file:
    for station_id, slug, family in stations:
        ...
        while current <= end:
            ...
            params = {"product": "water_temperature", "application": "openzl_public_datasets_collection", "begin_date": begin_date, "end_date": end_date, "station": station_id, "time_zone": "gmt", "units": "metric", "format": "json"}
...
while IFS='	' read -r station_id slug family begin_date end_date url rel_out; do
  ...
  if fetch "${url}" "${out}"; then
    success_count=$((success_count + 1)); say "ok ${station_id} ${slug} ${begin_date} ${end_date} ${out}"
  else
    status=$?
    if [ "${status}" -eq 2 ]; then cached_count=$((cached_count + 1)); say "cached ${station_id} ${slug} ${begin_date} ${end_date} ${out}"
    else failure_count=$((failure_count + 1)); rm -f "${out}" "${out}.tmp"; printf '%s\t%s\t%s\t%s\t%s\t%s\n' "${station_id}" "${slug}" "${begin_date}" "${end_date}" "${url}" "${rel_out}" >> "${FAILURES_FILE}"; say "failed ${station_id} ${slug} ${begin_date} ${end_date} ${url}"; fi
  fi
done < "${PLAN_FILE}"
