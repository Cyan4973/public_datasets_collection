...
series_defs = [
    {"series_id": "usgs_discharge_cfs_f64", "numeric_kind": "float", "bit_width": 64, "endianness": "little", "element_size_bytes": 8},
...
for site_id in site_ids:
    ...
    for year in range(2021, 2024):
        ...
        if stats_row["series_name"] != str(selected_series.get("name", "")):
            raise SystemExit(
                f"series name mismatch for {site_id} {year}: stats={stats_row['series_name']!r} raw={str(selected_series.get('name', ''))!r}"
            )
    ...
    for series in series_defs:
        ...
        expected_size = discharge_count * int(series["element_size_bytes"])
