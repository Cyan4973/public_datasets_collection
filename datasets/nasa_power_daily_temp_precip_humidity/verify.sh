...
DOWNLOAD_ROOT="${DATA_DIR}/downloads/nasa_power_daily_temp_precip_humidity"
FILTERED_ROOT="${DATA_DIR}/filtered/nasa_power_daily_temp_precip_humidity"
INDEX_ROOT="${DATA_DIR}/index/nasa_power_daily_temp_precip_humidity"
SAMPLES_ROOT="${DATA_DIR}/samples/nasa_power_daily_temp_precip_humidity"
LOG_ROOT="${DATA_DIR}/logs/nasa_power_daily_temp_precip_humidity"
...
parameter_ids = ["T2M", "PRECTOTCORR", "RH2M"]
series_defs = [
...
for (location_id, parameter_id), value_count in sorted(group_counts.items()):
    ...
    for series in series_defs:
        ...
        expected_records[(series["series_id"], sample_slug)] = {
            "dataset_id": "nasa_power_daily_temp_precip_humidity",
