...
DATASET_ID="appliances_energy_uci"
...
with (extract_dir / 'energydata_complete.csv').open(newline='') as fh:
...
(filter_dir / 'inventory.tsv').write_text(f'sample\trow_count\nenergydata_complete\t{row_count}\n')
rows=[]
for c in columns:
    sid='appl_' + c.lower().replace('.','').replace('/','_')
    ...
    rows.append({'dataset_id':'appliances_energy_uci','series_id':sid,'sample_path':str(out.relative_to(repo / data_dir)),'numeric_kind':'float','bit_width':64,'endianness':'little','element_size_bytes':8,'sample_size_bytes':out.stat().st_size,'value_count':len(values[c])})
