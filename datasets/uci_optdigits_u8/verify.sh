...
DATASET_ID = "uci_optdigits_u8"
PRIMARY_SERIES = {"optdigits_features"}
...
def inspect(path: Path, series_id: str) -> tuple[int, int]:
    ...
    with path.open("rb") as fh:
        for chunk in iter(lambda: fh.read(1 << 20), b""):
            ...
            if series_id == "optdigits_features" and any(value > 16 for value in chunk):
                raise SystemExit("feature value outside 0..16")
            if series_id == "optdigits_labels" and any(value > 9 for value in chunk):
                raise SystemExit("label value outside 0..9")
