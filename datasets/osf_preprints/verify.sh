...
allowed = {
    "osf_created_unix_u32": ("primary", "uint", 32, 4, "I"),
    "osf_modified_unix_u32": ("primary", "uint", 32, 4, "I"),
    "osf_published_unix_u32": ("primary", "uint", 32, 4, "I"),
}
...
if int(stats["complete_timestamp_records"]) != primary_counts[0]:
    raise SystemExit("complete timestamp count does not match sample count")

print(
    f"verified_samples={len(rows)} complete_timestamp_records={stats['complete_timestamp_records']} "
