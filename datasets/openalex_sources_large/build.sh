...
page_paths = sorted(
    [p for p in page_dir.glob("sources_page_*.json") if page_re.search(p.name)],
...
meta = {
    "openalex_source_works_count_u32": ("uint", 32, "I"),
    "openalex_source_oa_works_count_u32": ("uint", 32, "I"),
    "openalex_source_cited_by_count_u32": ("uint", 32, "I"),
    "openalex_source_2yr_mean_citedness_f32": ("float", 32, "f"),
    "openalex_source_h_index_u16": ("uint", 16, "H"),
    "openalex_source_i10_index_u32": ("uint", 32, "I"),
    "openalex_source_first_publication_year_u16": ("uint", 16, "H"),
    "openalex_source_last_publication_year_u16": ("uint", 16, "H"),
    "openalex_source_topic_count_u8": ("uint", 8, "B"),
}
...
def as_year(value: object, key: str) -> int:
    year = int(value)
    if year < 1000 or year > 3000:
        raise ValueError(f"{key} out of plausible year range: {year}")
    return year
...
for path in page_paths:
    ...
    for item in obj["results"]:
        rows_total += 1
        before = len(vals["openalex_source_works_count_u32"])
        try:
            entity_id = item["id"]
            if entity_id in seen_ids:
                raise ValueError(f"duplicate OpenAlex source id {entity_id}")
            seen_ids.add(entity_id)
            stats = item.get("summary_stats") or {}
            vals["openalex_source_works_count_u32"].append(as_uint(item["works_count"], "works_count", 0xFFFFFFFF))
            vals["openalex_source_oa_works_count_u32"].append(as_uint(item["oa_works_count"], "oa_works_count", 0xFFFFFFFF))
            vals["openalex_source_cited_by_count_u32"].append(as_uint(item["cited_by_count"], "cited_by_count", 0xFFFFFFFF))
            vals["openalex_source_2yr_mean_citedness_f32"].append(float(stats.get("2yr_mean_citedness") or 0.0))
            vals["openalex_source_h_index_u16"].append(as_uint(stats.get("h_index") or 0, "h_index", 0xFFFF))
            vals["openalex_source_i10_index_u32"].append(as_uint(stats.get("i10_index") or 0, "i10_index", 0xFFFFFFFF))
            vals["openalex_source_first_publication_year_u16"].append(as_year(item["first_publication_year"], "first_publication_year"))
            vals["openalex_source_last_publication_year_u16"].append(as_year(item["last_publication_year"], "last_publication_year"))
            vals["openalex_source_topic_count_u8"].append(count_array(item, "topics", 255))
        except Exception:
...
kept_rows = len(vals["openalex_source_works_count_u32"])
    ...
    rows.append(
        {
            ...
            "natural_record_kind": "openalex_source_row",
