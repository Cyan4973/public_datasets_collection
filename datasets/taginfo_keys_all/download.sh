...
BASE_URL="${TAGINFO_KEYS_URL:-https://taginfo.openstreetmap.org/api/4/keys/all}"
PAGE_SIZE="${TAGINFO_PAGE_SIZE:-500}"
MAX_PAGES="${TAGINFO_MAX_PAGES:-200}"
MAX_RECORDS="${TAGINFO_MAX_RECORDS:-100000}"
MIN_RECORDS="${TAGINFO_MIN_RECORDS:-2000}"
REQUEST_DELAY="${TAGINFO_REQUEST_DELAY_SECONDS:-0.2}"
UA="openzl-public-datasets/1.0 (numeric dataset collection)"
INVENTORY="$DOWNLOAD_DIR/download_inventory.json"
COMBINED="$DOWNLOAD_DIR/taginfo_keys_all.json"
...
if page_size < 1 or page_size > 1000:
    raise SystemExit("TAGINFO_PAGE_SIZE must be between 1 and 1000")
...
if record_count < min_records:
    raise SystemExit(f"only {record_count} records < TAGINFO_MIN_RECORDS={min_records}")
...
(page_dir / "taginfo_keys_all.json").write_text(
...
cp "$PAGE_DIR/taginfo_keys_all.json" "$COMBINED"
