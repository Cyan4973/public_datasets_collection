...
with stats_path.open("w", encoding="utf-8", newline="") as stats_file:
    ...
    for site_id in sorted(site_years):
        ...
        for year in sorted(site_years[site_id]):
            ...
            # Some sites return multiple wrappers; the first may be empty
            rows = None
...
samples_written = sum(1 for r in index_records if r["series_id"] == "usgs_dissolved_oxygen_f64")
