...
DOWNLOAD_ROOT="${DATA_DIR}/downloads/noaa_coops_water_temperature"
FILTERED_ROOT="${DATA_DIR}/filtered/noaa_coops_water_temperature"
INDEX_ROOT="${DATA_DIR}/index/noaa_coops_water_temperature"
SAMPLES_ROOT="${DATA_DIR}/samples/noaa_coops_water_temperature"
LOG_ROOT="${DATA_DIR}/logs/noaa_coops_water_temperature"
...
dataset_id = "noaa_coops_water_temperature"; series_id = "water_temperature_f64"; series_dir = samples_root / series_id; index_path = index_root / "samples.jsonl"; stats_path = filtered_root / "station_stats.tsv"
stations = [
...
series_dir.mkdir(parents=True, exist_ok=True); filtered_root.mkdir(parents=True, exist_ok=True); index_root.mkdir(parents=True, exist_ok=True)
def parse_station(station):
 station_dir = download_root / station["family"]
 if not station_dir.is_dir(): raise SystemExit(f"missing station raw directory: {station_dir}")
 by_timestamp = {}; json_files = sorted(station_dir.glob("*.json"))
 if not json_files: raise SystemExit(f"no JSON files found in {station_dir}")
 raw_file_count = raw_row_count = duplicate_rows = 0
 skipped_rows = 0
 for path in json_files:
  ...
  for row in rows:
   ...
   if timestamp in by_timestamp:
    duplicate_rows += 1
    if by_timestamp[timestamp] != value: raise SystemExit(f"{path} has conflicting duplicate timestamp {timestamp}")
   by_timestamp[timestamp] = value
 ordered = sorted(by_timestamp.items()); values = [value for _, value in ordered]
 return ordered, {"station_id": station["station_id"], "station_slug": station["station_slug"], "station_name": station["station_name"], "row_count": len(values), "raw_file_count": raw_file_count, "raw_row_count": raw_row_count, "duplicate_rows": duplicate_rows, "skipped_rows": skipped_rows, "first_timestamp_utc": ordered[0][0], "last_timestamp_utc": ordered[-1][0], "min_value": min(values), "max_value": max(values)}
index_records = []
with stats_path.open("w", encoding="utf-8", newline="") as stats_file:
 writer = csv.writer(stats_file, delimiter="\t"); writer.writerow(["station_id","station_slug","station_name","row_count","raw_file_count","raw_row_count","duplicate_rows","skipped_rows","first_timestamp_utc","last_timestamp_utc","min_value","max_value"])
 for station in stations:
  ordered, stats = parse_station(station); payload = array.array("d", [value for _, value in ordered])
  if os.sys.byteorder != "little": payload.byteswap()
