# Open-Meteo Hourly Relative_Humidity
...
- `download.sh` validates that each JSON payload contains `hourly.time` and
  `hourly.relative_humidity_2m` with matching lengths before accepting it into cache.
...
```sh
bash datasets/open_meteo_hourly_relative_humidity/download.sh
bash datasets/open_meteo_hourly_relative_humidity/build.sh
bash datasets/open_meteo_hourly_relative_humidity/verify.sh
```
