...
def parse_ts(s:str)->int:
    return calendar.timegm(__import__("datetime").datetime.strptime(s, "%Y-%m-%dT%H:%M:%SZ").utctimetuple())
for event in events:
    try:
...
for sid,values in vals.items():
    ...
    rows.append({"dataset_id":"nasa_eonet_events","series_id":sid,"sample_path":out.relative_to(data_root).as_posix(),"numeric_kind":kind,"bit_width":bits,"endianness":"little","element_size_bytes":bits//8,"sample_size_bytes":out.stat().st_size,"value_count":len(values)})
stats={"dataset_id":"nasa_eonet_events","rows_total":len(events),"rows_skipped":skipped,"series":{sid:{"kept_rows":len(vals[sid])} for sid in vals}}
