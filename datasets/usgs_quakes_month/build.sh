...
obj = json.load(open(download_dir / 'usgs_quakes_month.json', encoding='utf-8'))['features']
meta = {
    ...
    'usgs_quake_depth_km': ('float', 32, 'f'),
}
...
for row in obj:
    try:
        p = row['properties']; coords = row['geometry']['coordinates']
        ...
        vals['usgs_quake_depth_km'].append(float(coords[2]))
    except Exception:
        skipped += 1
rows = []
for sid, (kind, bits, code) in meta.items():
    ...
    rows.append({'dataset_id': 'usgs_quakes_month', 'series_id': sid, 'sample_path': out.relative_to(data_root).as_posix(), 'numeric_kind': kind, 'bit_width': bits, 'endianness': 'little', 'element_size_bytes': bits // 8, 'sample_size_bytes': out.stat().st_size, 'value_count': len(values)})
(filter_dir / 'ingest_stats.json').write_text(json.dumps({'dataset_id': 'usgs_quakes_month', 'rows_total': len(obj), 'rows_skipped': skipped}, indent=2, sort_keys=True) + '\n', encoding='utf-8')
