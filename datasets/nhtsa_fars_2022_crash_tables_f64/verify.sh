...
DATASET_ID = "nhtsa_fars_2022_crash_tables_f64"
MIN_SAMPLES = 40
MIN_PRIMARY_VALUES = 2_000_000
MIN_PRIMARY_BYTES = 16_000_000
MIN_MEDIAN_VALUES = 30_000
MAX_PRIMARY_BYTES = 1_000_000_000
EXCLUDED_FIELDS = {"ST_CASE", "VEH_NO", "PER_NO"}
...
for row in primary:
    ...
    if row.get("sample_geometry") != "table_column" or row.get("natural_record_kind") != "nhtsa_fars_numeric_table_column":
        raise SystemExit(f"unexpected sample metadata: {row}")
    if row.get("source_field") in EXCLUDED_FIELDS:
        raise SystemExit(f"identifier field emitted: {row}")
    ...
    fields.add((row["source_table"], row["source_field"]))
...
if tables != {"ACCIDENT", "PERSON", "VEHICLE"}:
    raise SystemExit(f"missing expected table coverage: {sorted(tables)}")

print(
    f"verified dataset={DATASET_ID} samples={len(primary)} fields={len(fields)} "
    f"tables={len(tables)} median_values={int(median_values)} "
    f"total_values={total_values} total_bytes={total_bytes}"
)
PY
