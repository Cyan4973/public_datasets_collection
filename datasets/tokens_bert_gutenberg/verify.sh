...
if not (download_dir / "vocab.txt").is_file():
    raise SystemExit("missing vocab.txt")
...
if stats["vocab_size"] < 30000:
    raise SystemExit("unexpected vocab size")

for row in rows:
    if row["dataset_id"] != "tokens_bert_gutenberg" or row["series_id"] != "tokens_bert_ids":
