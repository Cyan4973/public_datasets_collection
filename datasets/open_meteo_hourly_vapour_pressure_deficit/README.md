# Open-Meteo Hourly vapour pressure deficit
...
- `download.sh` validates that each JSON payload contains `hourly.time` and
  `hourly.vapour_pressure_deficit` with matching lengths before accepting it into cache.
...
```sh
bash datasets/open_meteo_hourly_vapour_pressure_deficit/download.sh
bash datasets/open_meteo_hourly_vapour_pressure_deficit/build.sh
bash datasets/open_meteo_hourly_vapour_pressure_deficit/verify.sh
```
